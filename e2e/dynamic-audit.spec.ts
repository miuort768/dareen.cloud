...
test.describe('Dynamic UI Audit (report only — no fixes)', () => {
  for (const role of ROLES) {
    test(`audit role: ${role.id}`, async ({ page }, testInfo) => {
      ...
      console.log(
        `[AUDIT] ${testInfo.project.name}/${role.id}: routes=${ROUTES[role.id].length} covered=${coveredTotal} pe:none=${peTotal} broken=${broken} report=${reportFile}`,
      )

      for (const e of entries) {
        if (e.viteError)
          console.log(
            `  !! ${role.id} ${e.route} -> VITE RUNTIME ERROR: ${e.viteError.slice(0, 260)}`,
          )
        if (e.errorBoundary)
          console.log(`  !! ${role.id} ${e.route} -> ERROR BOUNDARY (${e.pathname})`)
        if (e.redirectedToLogin)
          console.log(`  !! ${role.id} ${e.route} -> REDIRECTED TO LOGIN (${e.pathname})`)
        if (e.noAccess)
          console.log(`  ?? ${role.id} ${e.route} -> no-access/403 text shown (${e.pathname})`)
        if (e.spinnerVisible && e.interactiveCount === 0)
          console.log(`  ?? ${role.id} ${e.route} -> stuck spinner, 0 interactive elements`)
        for (const it of e.covered.slice(0, 10)) {
          const label = it.text || it.aria || it.href || it.cls.slice(0, 40)
          const ov = it.overlay!
          console.log(
            `  X ${role.id} ${e.route}: <${it.tag}> "${label}" COVERED by <${ov.tag}> z=${ov.z} pos=${ov.pos} pe=${ov.pe} cls=${ov.cls.slice(0, 70)}`,
          )
        }
        if (e.covered.length > 10)
          console.log(`    ... ${role.id} ${e.route}: +${e.covered.length - 10} more covered`)
        for (const it of e.pointerEventsNone.slice(0, 5)) {
          const label = it.text || it.aria || it.href || it.cls.slice(0, 40)
          console.log(
            `  P ${role.id} ${e.route}: <${it.tag}> "${label}" has pointer-events:none (not disabled)`,
          )
        }
      }
    })
