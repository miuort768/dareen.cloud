...
async function login(page: Page, username: string, password: string): Promise<string> {
  ...
  } catch { /* login failed or server unreachable */ }
...
async function auditRoute(page: Page, route: string): Promise<AuditEntry> {
  ...
  page.on('requestfailed', (req) => {
    const f = req.failure();
    requestFailures.push({ kind: 'fail', url: req.url().slice(0, 180), error: (f && f.errorText || '').slice(0, 120) });
  });
  ...
  for (const role of ROLES) {
    test(`review role: ${role.id}`, async ({ page }, testInfo) => {
      ...
      entries.push({
        ...
        interactiveCount: 0, dialogCount: 0, modalOpenerButtons: 0,
        viteError: '', consoleErrors: [], pageErrors: [], badResponses: [], requestFailures: [],
        covered: [], pointerEventsNone: [],
      });
      ...
      const reportFile = path.join(REPORT_DIR, `final-review-${testInfo.project.name}-${role.id}.json`);
      ...
      console.log(`[REVIEW] ${testInfo.project.name}/${role.id}: routes=${ROUTES[role.id].length} consoleErrors=${consoleTotal} pageErrors=${pageErrTotal} badResponses=${badResp} requestFailures=${reqFail} broken=${broken} report=${reportFile}`);

      for (const e of entries) {
        if (e.viteError) console.log(`  !! ${role.id} ${e.route} -> VITE ERROR: ${e.viteError.slice(0, 200)}`);
        ...
        if (e.noAccess) console.log(`  ?? ${role.id} ${e.route} -> no-access/403 text (${e.pathname})`);
        if (e.spinnerVisible && e.interactiveCount === 0) console.log(`  ?? ${role.id} ${e.route} -> stuck spinner, 0 interactive`);
        for (const ce of e.consoleErrors.slice(0, 5)) console.log(`  C ${role.id} ${e.route}: ${ce.text.slice(0, 160)}`);
        if (e.consoleErrors.length > 5) console.log(`    ... +${e.consoleErrors.length - 5} console errors`);
        for (const pe of e.pageErrors.slice(0, 5)) console.log(`  E ${role.id} ${e.route}: ${pe.slice(0, 160)}`);
        for (const br of e.badResponses.slice(0, 8)) console.log(`  N ${role.id} ${e.route}: ${br.kind} ${br.url}`);
        if (e.badResponses.length > 8) console.log(`    ... +${e.badResponses.length - 8} bad responses`);
