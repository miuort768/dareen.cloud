...
const Root = ({ children, error, required, size = 'md', className }: FormFieldProps) => {
  ...
  return (
    <FormFieldContext.Provider value={{ id, error, required, size }}>
      <div className={cn('flex w-full flex-col gap-1.5 text-start', className)} dir="rtl">
        {children}
      </div>
...
const ErrorMsg = ({ children, className }: ErrorProps) => (
  <span className={cn('flex items-center gap-1 text-xs font-bold text-error', className)}>
    <svg
