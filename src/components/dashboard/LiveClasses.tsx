...
import { PlayCircle, Users, Monitor, Loader2, Radio, Plus, AlertCircle, RefreshCcw } from 'lucide-react';
        ...
        <div className="space-y-4" dir="rtl">
            <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100/50 dark:border-slate-800/50 transition-all duration-300 hover:shadow-md">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[9px] font-medium text-slate-500 dark:text-slate-400">مركز التفاعل المباشر</p>
                    </div>
                </div>
                ...
                {isTeacher && (
                    <button
                        ...
                        disabled={starting}
                        className="text-white px-5 py-2 text-[10px] font-bold rounded-2xl transition-all active:scale-[0.97] flex items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed shadow-sm hover:shadow-md bg-indigo-600 hover:bg-indigo-700"
                    >
                        {starting ? (
            ...
            {loading ? (
                ...
                <div className="p-10 text-center bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100/50 dark:border-slate-800/50 transition-all">
                    <Monitor size={36} strokeWidth={1.5} className="mx-auto mb-3 text-indigo-300 dark:text-indigo-600" />
                    <p className="font-bold text-[11px] text-indigo-600 dark:text-indigo-400">لا يوجد بث مباشر متاح حالياً</p>
                    {isTeacher && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {sessions.map(session => (
                        ...
                        >
                            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl shadow-sm text-white text-[8px] font-bold w-fit bg-indigo-600">
                            ...
                            <button
                                ...
                                className="w-full text-white py-3 text-[10px] font-bold rounded-2xl flex items-center justify-center gap-2 transition-all shadow-sm hover:shadow-md active:scale-[0.97] bg-indigo-600 hover:bg-indigo-700"
                            >
