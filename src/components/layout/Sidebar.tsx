...
export const Sidebar = () => {
                    ...
                    <div className={cn(
                        ...
                        <div className={cn("flex items-center gap-3 overflow-hidden whitespace-nowrap", collapsed && "gap-0")}>
                            ...
                            <span className={cn(
                                ...
                                collapsed ? "w-0 opacity-0 overflow-hidden" : "w-auto opacity-100 ps-3"
                            )}>
                                دارين السابعة
                            </span>
                        </div>
    ...
    return (
        <>
            <div
                className={cn(
                    "hidden lg:flex bg-card h-screen border-s border-border transition-all duration-300 flex-col fixed top-0 end-0 z-50 shrink-0",
                    collapsed ? "w-20" : "w-72"
                )}
            >
                <div className={cn(
                        ...
                        >
                            <div className="relative shrink-0">
                                ...
                                {item.id === 'chat' && totalUnreadCount > 0 && (
                                    <span className="absolute -top-1.5 -end-1.5 w-4 h-4 flex items-center justify-center bg-error text-on-error text-micro font-medium rounded-full animate-pulse shadow-sm border border-border">
                            ...
                            {collapsed && (
                                <div className="absolute start-full top-1/2 -translate-y-1/2 rtl:ms-2 rtl:start-full ltr:me-2 ltr:start-auto ltr:end-full px-2 py-1 bg-surface text-main text-xs rounded-none opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 shadow-lg">
                                    {item.name}
                                </div>
                            )}
                        </NavLink>
            ...
            <div className={cn(
                "lg:hidden fixed bottom-0 start-0 end-0 h-[70px] bg-card/95 backdrop-blur-xl border-t border-border flex items-center justify-around px-2 z-[100] overflow-hidden max-w-full transition-transform duration-300",
                        ...
                        {({ isActive }) => (
                            <div className="flex items-center gap-2 relative">
                                ...
                                <div className="relative">
                                    ...
                                    {item.id === 'chat' && totalUnreadCount > 0 && (
                                        <span className="absolute -top-2 -end-2 w-5 h-5 bg-error text-on-error text-micro font-medium flex items-center justify-center rounded-full ring-2 ring-border shadow-sm md:animate-pulse">
            ...
            <div className={cn(
                ...
                />
                <div className={cn(
                    "absolute bottom-0 start-0 end-0 bg-card p-4 transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden max-h-[90vh] flex flex-col border-t border-white/10 w-full max-w-full",
                                    ...
                                    {({ isActive }) => (
                                        <>
                                            <div className={cn(
                                                ...
                                                <item.icon size={14} />
                                                {item.id === 'chat' && totalUnreadCount > 0 && (
                                                    <span className="absolute -top-1 -end-1 w-3.5 h-3.5 flex items-center justify-center bg-error text-on-error text-micro font-medium rounded-full shadow-sm border border-border">
                                                        {totalUnreadCount > 9 ? '9+' : totalUnreadCount}
                                                    </span>
                                                )}
                                            </div>
