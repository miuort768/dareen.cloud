      ...
      <header className="md:hidden flex items-center justify-between px-2 pt-3 pb-2">
        <div className="flex items-center gap-1">
          ...
          <div>
            ...
            <p className="text-[9px] font-bold leading-tight text-indigo-950 dark:text-indigo-100">أفضل مدرسة افتراضية</p>
            <p className="text-[7px] text-indigo-600 dark:text-indigo-400 leading-tight mt-0.5">Dareen for Education & Online Learning</p>
          </div>
        </Link>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label={theme === 'dark' ? 'الوضع النهاري' : 'الوضع الليلي'} className="w-10 h-10 rounded-full bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 flex items-center justify-center shrink-0">
