...
export const MobileHeader = ({ hideThemeToggle }: { hideThemeToggle?: boolean }) => {
  ...
  const navItems = [
    { label: '«·—∆Ì”Ì…', path: '/', icon: Home },
    { label: '«·œÊ—« ', path: '/courses', icon: BookOpen },
    { label: '«·„ﬂ »…', path: '/books', icon: Book },
    { label: '„‰ ‰Õ‰', path: '/about', icon: Info },
    { label: '« ’· »‰«', path: '/contact', icon: Phone },
  ];
      ...
      <header className="md:hidden flex items-center justify-between px-2 pt-3 pb-2 bg-surface dark:bg-background">
        <div className="flex items-center gap-1">
          ...
          <div>
                            ...
                            <p className="text-micro font-bold leading-tight text-main dark:text-muted">√›÷· „œ—”… «› —«÷Ì…</p>
                            <p className="text-micro text-main dark:text-muted leading-tight mt-0.5">Dareen for Education & Online Learning</p>
          </div>
        </Link>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label={theme === 'dark' ? '«·Ê÷⁄ «·‰Â«—Ì' : '«·Ê÷⁄ «··Ì·Ì'} className={`w-10 h-10 rounded-full bg-surface dark:bg-card shadow-sm border border-border dark:border-primary/30 flex items-center justify-center shrink-0 ${hideThemeToggle ? 'hidden' : ''}`}>
            {theme === 'dark' ? <Sun className="w-4 h-4 text-accent" /> : <Bed className="w-4 h-4 text-primary" />}
          </button>
          <div className="relative">
          <button onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? '≈€·«ﬁ «·ﬁ«∆„…' : '› Õ «·ﬁ«∆„…'} className="w-10 h-10 rounded-full bg-surface dark:bg-card shadow-sm border border-border dark:border-primary/30 flex items-center justify-center">
