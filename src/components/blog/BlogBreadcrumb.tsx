    ...
    const content = (
        <>
            ...
            <div className="flex gap-2 mb-6">
                {showChangeButton && (
                    <button type="button" onClick={onBack} className="inline-flex items-center gap-1.5 px-4 py-2 bg-card border border-border text-main hover:border-primary/40 hover:text-primary text-xs font-extrabold rounded-xl transition-all duration-200">
                        <ArrowLeft size={14} /><span>تغيير المادة</span>
                    </button>
                )}
                <button type="button" onClick={onHome} className="w-full sm:w-auto inline-flex items-center justify-center sm:justify-start gap-1.5 px-4 py-2 bg-primary text-on-primary hover:bg-primary-hover text-xs font-extrabold rounded-xl transition-all duration-200">
