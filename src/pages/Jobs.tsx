                            ...
                            <div className="flex items-center gap-2">
                                {steps.map((s, i) => (
                                    <div key={s.id} className="flex-1 flex items-center gap-2">
                                        ...
                                        <span className={`hidden md:block text-[9px] font-bold transition-colors ${step === s.id ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>{s.title}</span>
                                        {i < steps.length - 1 && <div className={`flex-1 h-0.5 mx-1 transition-colors duration-500 ${step > s.id ? 'bg-emerald-400' : 'bg-slate-200 dark:bg-slate-700'}`} />}
                                    </div>
                                ))}
                            </div>
                        </div>
