      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="lg:w-72 shrink-0">
          <div className="relative mb-3">
            ...
            <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="بحث عن معلمة..." className="w-full pr-9 pl-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-none text-xs font-normal focus:outline-none focus:ring-2 focus:ring-indigo-500/30" />
          </div>
          <div className="space-y-1 max-h-[400px] overflow-y-auto">
            {filteredTeachers.map((t: { id: string; name: string; subject: string }) => (
              <button key={t.id} onClick={() => setSelectedTeacher(t.id)} className={cn("w-full text-right px-3 py-2 rounded-none text-xs font-normal transition-all", selectedTeacher === t.id ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800' : 'bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800')}>
                <span>{t.name}</span>
        ...
        <div className="flex-1">
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { if (selTeacher) saveMutation.mutate({ teacherId: selTeacher.id, teacherName: selTeacher.name, slots: editingSlots }); }} disabled={!hasChanges || saveMutation.isPending} className={cn("flex items-center gap-2 px-4 py-2 rounded-none text-xs font-normal transition-all shadow-sm active:scale-95", hasChanges && !saveMutation.isPending ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 cursor-not-allowed')}>
            ...
            <div className="space-y-2">
              {dayNumbers.map(day => {
                ...
                return (
                  <div key={day} className={cn("flex items-center gap-3 p-3 rounded-none border transition-all", isAvail === 1 ? 'bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-900/30' : isAvail === 0 ? 'bg-rose-50/50 dark:bg-rose-900/10 border-rose-200 dark:border-rose-900/30' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800')}>
                    <button onClick={() => toggleDay(day)} className={cn("shrink-0 p-1.5 rounded-none transition-all", isAvail === 1 ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600' : isAvail === 0 ? 'bg-rose-100 dark:bg-rose-900/40 text-rose-600' : 'bg-slate-100 dark:bg-slate-800 text-slate-400')}>
                    ...
                    {slot ? <>
                      <input type="time" value={slot.startTime} onChange={e => updateSlot(day, 'startTime', e.target.value)} className={cn("w-24 px-2 py-1.5 rounded-none text-xs font-normal border focus:outline-none", isAvail === 1 ? 'bg-white dark:bg-slate-800 border-emerald-200 dark:border-emerald-900' : isAvail === 0 ? 'bg-white dark:bg-slate-800 border-rose-200 dark:border-rose-900' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700')} />
                      <span className="text-slate-400 text-xs">–</span>
                      <input type="time" value={slot.endTime} onChange={e => updateSlot(day, 'endTime', e.target.value)} className={cn("w-24 px-2 py-1.5 rounded-none text-xs font-normal border focus:outline-none", isAvail === 1 ? 'bg-white dark:bg-slate-800 border-emerald-200 dark:border-emerald-900' : isAvail === 0 ? 'bg-white dark:bg-slate-800 border-rose-200 dark:border-rose-900' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700')} />
