    ...
    <header className="container mx-auto px-4 max-w-4xl mb-6 md:mb-12">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3 md:gap-0 mb-3 md:mb-6">
            <div className="order-2 md:order-1 flex flex-wrap gap-4 items-center">
                <span className="bg-error-light text-error font-black text-xs px-3 py-1.5 uppercase tracking-widest">{post.category}</span>
                <div className="flex items-center gap-4 text-xs text-muted font-medium">
                    ...
                    <div className="bg-error-light text-error font-black text-xs sm:text-xs px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg"><User size={12} className="inline" /> {post.author}</div>
            ...
            <Link to="/books" className="order-1 md:order-2 w-full md:w-auto inline-flex items-center justify-center md:justify-start gap-2 px-4 py-3 bg-hover text-main hover:bg-primary hover:text-on-primary transition-all font-bold text-sm rounded-card">
                <ArrowRight size={16} /><span>العودة لجميع المقالات</span>
            </Link>
        </div>
        <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-heading font-black text-main leading-tight mb-2 md:mb-4">{post.title}</h1>
        {post.contentType !== 'more' && post.contentType !== 'foundation' && (post.curriculum || post.level || post.grade || post.term || post.subject) && (
            <div className="flex flex-wrap gap-2 mb-4">
                {post.curriculum && <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-info-light text-info text-xs font-bold rounded-lg border border-info/50"><BookOpen size={12} />{curriculumNames[post.curriculum] || post.curriculum}</span>}
                {post.level && <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-success-light text-success text-xs font-bold rounded-lg border border-success/50"><GraduationCap size={12} />{levelNames[post.level] || post.level}</span>}
                {post.grade && <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-primary-soft text-primary text-xs font-bold rounded-lg border border-primary/50"><School size={12} />الصف {gradeNames[post.grade] || post.grade}</span>}
                {post.term && <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-warning-light text-warning text-xs font-bold rounded-lg border border-warning/50"><Tag size={12} />{termNames[post.term] || post.term}</span>}
                {post.subject && <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-error-light text-error text-xs font-bold rounded-lg border border-error/50"><BookOpen size={12} />{subjectNames[post.subject] || post.subject}</span>}
            </div>
        )}
        {post.tags && (() => {
            ...
            return (
                <div className="flex flex-wrap gap-2 mb-4">
                    {tags.map((tag: string, i: number) => <span key={`tag-${i}`} className="text-micro font-bold text-muted bg-surface px-2 py-1 rounded-lg">#{tag}</span>)}
                </div>
            );
        })()}
    </header>
