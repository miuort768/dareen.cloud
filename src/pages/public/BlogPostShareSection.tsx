    ...
    return (
        <div className="border-t border-border dark:border-border pt-8 mt-0 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-3">
                ...
                <button onClick={() => { navigator.clipboard.writeText(url).then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500); }); }} className="w-9 h-9 rounded-full bg-gradient-to-br from-[var(--bg-primary)] via-[var(--bg-primary)] to-[var(--bg-warning)] flex items-center justify-center hover:opacity-80 transition-all text-on-primary" title="انسخ الرابط">
                    {copied ? <span className="text-[10px] font-black">تم</span> : <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>}
                </button>
                <button onClick={() => { navigator.clipboard.writeText(url).then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500); }).catch(() => { if (navigator.share) navigator.share({ title: post.title, url }); }); }} className="w-9 h-9 rounded-full bg-background0 flex items-center justify-center hover:opacity-80 transition-all text-on-primary" title="نسخ الرابط">
                    {copied ? <span className="text-[10px] font-black">تم</span> : <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>}
                </button>
            </div>
            <div className="flex items-center gap-3">
