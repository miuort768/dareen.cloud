      ...
      <script type="application/ld+json">
        {safeJsonLd({
          ...
          image: post.coverImage?.startsWith('http')
            ? post.coverImage
            : `https://dareen.cloud${post.coverImage || ''}`,
          ...
          mainEntityOfPage: {
            '@type': 'WebPage',
            '@id': `https://dareen.cloud/books/${post.slug}`,
          },
        })}
      </script>
      <MobileHeader />

      <main className="relative flex-grow pb-6 pt-3 md:pb-10 md:pt-32">
              ...
              <div className="my-8 flex flex-wrap justify-center gap-3">
                {downloadLink && (
                  ...
                  >
                    <Download size={16} />
                    <span>
                      {buttonState?.type === 'download' && buttonState.phase === 'counting'
                        ? `${post.downloadButtonText || post.download_button_text || 'تحميل الملف'} (${buttonState.seconds})`
                        : buttonState?.type === 'download' && buttonState.phase === 'ready'
                          ? 'الملف جاهز'
                          : post.downloadButtonText || post.download_button_text || 'تحميل الملف'}
                    </span>
                  </button>
                )}
                {watchLink && (
                  <button
                    ...
                    className={`inline-flex flex-1 items-center justify-center gap-2 whitespace-nowrap rounded-card px-6 py-3 text-xs font-black shadow-elevation-3 outline-none transition-all hover:bg-error-active focus-visible:ring-2 focus-visible:ring-focus disabled:cursor-not-allowed disabled:opacity-50 sm:text-sm md:px-16 md:py-4 ${buttonState?.type === 'watch' && buttonState.phase === 'counting' ? 'bg-success text-on-success' : buttonState?.type === 'watch' && buttonState.phase === 'ready' ? 'bg-success text-on-success ring-2 ring-success ring-offset-2' : 'bg-error text-on-error'}`}
                  >
                    <Eye size={16} />
                    <span>
                      {buttonState?.type === 'watch' && buttonState.phase === 'counting'
                        ? `${post.watchButtonText || post.watch_button_text || 'مشاهدة الملف'} (${buttonState.seconds})`
                        : buttonState?.type === 'watch' && buttonState.phase === 'ready'
                          ? 'الملف جاهز'
                          : post.watchButtonText || post.watch_button_text || 'مشاهدة الملف'}
                    </span>
                  </button>
                )}
              </div>
