            ...
            <div className="bg-card border border-border shadow-lg w-full max-w-xl overflow-hidden rounded-2xl md:animate-in md:zoom-in-95 md:duration-200">
                <div className="bg-gradient-to-l from-[var(--bg-primary)] to-[var(--bg-primary-hover)] text-on-primary p-5 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-medium text-on-primary/70 tracking-wider">سجل مالي معتمد • {month}</p>
                        </div>
                    </div>
                ...
                <div className="p-6 space-y-6 overflow-y-auto max-h-[80vh]">
                    <div className="flex justify-between items-start pb-6 border-b border-border">
                        <div>
                            <p className="text-[10px] font-bold text-muted uppercase mb-1">المعلمة</p>
                            <h3 className="text-lg font-black text-main">{teacher.name}</h3>
                            <span className="text-[10px] font-bold px-2 py-0.5 inline-block mt-1 rounded-lg bg-primary-soft text-primary">{teacher.subject}</span>
                        </div>
                        <div className="text-left">
                            <p className="text-[10px] font-bold text-muted uppercase mb-1">التاريخ</p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-card p-4 border border-border rounded-xl">
                            <p className="text-[10px] font-bold text-muted uppercase mb-1">إجمالي الحصص</p>
                        ...
                        <div className="p-4 border border-primary-soft rounded-xl bg-primary-soft/30">
                            <p className="text-[10px] font-bold uppercase mb-1 text-primary">صافي المستحق</p>
                            <div className="flex items-baseline gap-1">
                                <p className="text-2xl font-black font-mono text-primary">{teacher.totalAmount.toLocaleString()}</p>
                                <span className="text-[10px] font-bold uppercase text-primary">{CURRENCY_SYMBOL}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        {teacher.sessionsList?.length > 10 && (
                            <p className="text-[10px] text-center text-dim mt-2 italic">... و {teacher.sessionsList.length - 10} حصص أخرى في السجل</p>
                        )}
                    </div>
