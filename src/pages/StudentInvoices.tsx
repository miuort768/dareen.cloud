...
export const StudentInvoices = () => {
    ...
    const handleStudentChange = (studentId: string) => {
        ...
        if (student) {
            ...
            const items = studentSessions.map((sess: { id: string; date: string; studentName?: string; teacherName?: string; price?: number; subject?: string; status?: string }) => ({
                description: `${sess.subject} - ${sess.teacherName} (${sess.status === 'completed' ? 'Õ÷Ê—' : '€Ì«»'})`,
            ...
            setFormData({
                ...formData,
                studentId,
                description: subjects ? `—”Ê„: ${subjects}` : '—”Ê„ ‘Â—Ì…',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!student) {
            showNotification('Œÿ√: Ì—ÃÏ «Œ Ì«— ÿ«·» ’ÕÌÕ', 'error');
            setIsSaving(false);
            return;
        }
        ...
        try {
            ...
            fetchData();
            handleCancel();
            showNotification(editingId ? ' „  ÕœÌÀ «·›« Ê—… »‰Ã«Õ' : ' „ ≈’œ«— «·›« Ê—… »‰Ã«Õ', 'success');
        } catch (error) {
            console.error('Error saving invoice:', error);
            const errorMessage = error.response?.data?.error || error.message || 'ÕœÀ Œÿ√ √À‰«¡ Õ›Ÿ «·»Ì«‰« ';
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            showNotification(error.message || '›‘· ›Ì Õ–› «·›« Ê—…', 'error');
        } finally {
            setDeletingId(null);
        }
    };
    ...
    const handleDeleteAll = async () => {
        ...
        try {
            ...
            fetchData();
            showNotification(' „ Õ–› Ã„Ì⁄ ›Ê« Ì— «·ÿ·«» »‰Ã«Õ', 'success');
        } catch (error) {
            console.error('Error deleting all invoices:', error);
            showNotification('ÕœÀ Œÿ√ √À‰«¡ „Õ«Ê·… Õ–› «·ﬂ·', 'error');
        } finally {
            setLoading(false);
            ...
            <div className="mx-auto px-2 space-y-4">
                ...
                <div className="bg-primary rounded-2xl px-4 md:px-6 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg md:text-xl font-black text-on-primary leading-tight">›Ê« Ì— Ê Õ’Ì· «·ÿ·«»</h1>
                            <p className="text-xs font-bold text-on-primary opacity-70 mt-0.5">≈œ«—… «· œ›ﬁ«  «·‰ﬁœÌ… Ê«·„” Õﬁ«  «·œ—«”Ì…</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold px-3 py-2 whitespace-nowrap rounded-xl bg-success text-on-success">
                        <Sparkles size={13} />
                        {totalRevenue.toLocaleString()} Ã.„ ≈Ã„«·Ì «·„Õ’·
                    </div>
                </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-3 items-center justify-between">
                        <div className="flex-1 flex gap-3 items-center w-full">
                            ...
                            >
                                ...
                                <option value="paid">„œ›Ê⁄…</option>
                                <option value="pending">„⁄·ﬁ…</option>
                                <option value="overdue">„ √Œ—…</option>
                            </select>
                        </div>
                        ...
                        <div className="flex items-center gap-2 w-full lg:w-auto overflow-x-auto no-scrollbar pb-1 lg:pb-0">
                            ...
                            <SecondaryBtn onClick={handleImportStudents} title="«” Ì—«œ „‰ ”Ã· «·Õ’’">
                                <UserPlus size={14} /> «” Ì—«œ
                            </SecondaryBtn>
                            <SecondaryBtn onClick={() => window.print()} title="ÿ»«⁄… «·”Ã·">
                                <Printer size={14} />
                            </SecondaryBtn>
                            <DangerBtn onClick={() => setDeleteAllModalOpen(true)} title="Õ–› «·ﬂ·">
                                <Trash2 size={14} />
                            </DangerBtn>
                        </div>
                    </div>
