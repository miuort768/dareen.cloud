    ...
    <div className="relative z-10 max-w-[1600px] mx-auto px-2">
            ...
            <div className="shadow-sm p-3 flex flex-col md:flex-row gap-4 items-center justify-between rounded-none mb-6" style={{ backgroundColor: 'var(--bg-warning)' }}>
                ...
                <div className="relative w-full md:w-64 shrink-0">
                    <Search className="absolute end-3 top-1/2 -translate-y-1/2 text-white/50" size={14} />
                    <input
                        type="text"
                        placeholder="بحث عن طالب..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full bg-white/15 border border-white/20 text-on-primary placeholder:text-white/50 pe-10 py-2 text-xs font-bold outline-none rounded-none"
                    />
                </div>
            </div>
                ...
                {scheduledAppointments.length > 0 ? (
                    scheduledAppointments.map((app) => (
                        <div key={app.id} className={cn(
                            ...
                            {/* Status Stripe */}
                            <div className={cn(
                                "absolute top-0 end-0 w-1.5 h-full transition-all",
