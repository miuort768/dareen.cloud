...
export const Agenda = () => {
    ...
    const logAttendanceMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            showNotification(' „  ”ÃÌ· «·Õ’… »‰Ã«Õ', 'success');
        },
        onError: () => {
            showNotification('›‘·  ”ÃÌ· «·Õ’…', 'error');
        }
    });
                ...
                enrollment.schedule?.forEach(slot => {
                    if (slot.day === activeDay) {
                        ...
                        list.push({
                            ...
                            time: `${slot.hour} ${slot.period === 'am' ? '’»«Õ«' : '„”«¡'}`,
                            hour: slot.hour,
                            period: slot.period,
                            isDone,
                            enrollment
    ...
    if (loadingStudents || loadingSessions) {
        return <div className="p-12 text-center">Ã«—Ì  Õ„Ì· «·√Ã‰œ…...</div>;
    }
    ...
    <div className="relative z-10 max-w-[1600px] mx-auto px-2">
            <PageHeader
                title="√Ã‰œ… «·Õ’’ «·ÌÊ„Ì…"
                subtitle="„ «»⁄… Ê ‰›Ì– «·Õ’’ «·„ÃœÊ·… ··ÌÊ„"
                icon={CalendarCheck}
                color="amber"
                stats={[
                    { label: 'Õ’’ «·ÌÊ„', value: scheduledAppointments.length },
                    { label: ' „ «· ‰›Ì–', value: scheduledAppointments.filter(a => a.isDone).length }
                ]}
            />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {scheduledAppointments.length > 0 ? (
                            ...
                            <div className="p-5 space-y-4">
                                ...
                                <div className="pt-2">
                                    {app.isDone ? (
                                        ...
                                        >
                                            {logAttendanceMutation.isPending ? 'Ã«—Ì «· ”ÃÌ·...' : (
                                                <>
                                                    <CheckCircle2 size={16} />
                                                     √ﬂÌœ «·≈‰Ã«“
                                                </>
                    ...
                    <div className="col-span-full py-24 text-center border-2 border-dashed border-gray-200 dark:border-gray-800 bg-white/50 dark:bg-gray-900/50">
                        ...
                        <h3 className="text-lg font-medium text-gray-400">·«  ÊÃœ Õ’’ „ÃœÊ·… ·Â–« «·ÌÊ„</h3>
                        <p className="text-sm text-gray-400 font-normal mt-1 uppercase tracking-widest">Ì—ÃÏ «· √ﬂœ „‰ «·Ãœ«Ê· «·œ—«”Ì…</p>
                    </div>
                )}
            </div>
        </div>
