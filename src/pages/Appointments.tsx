            ...
            <div className="bg-card border border-border shadow-sm rounded-2xl mb-4">
                ...
                <div className="p-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="relative">
                        <Search size={13} className="absolute end-3 top-1/2 -translate-y-1/2 text-dim" />
                        <input
                            type="text"
                            placeholder="ابحث باسم الطالب أو المادة..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pe-8 ps-8 py-2 border border-border text-xs font-bold outline-none focus:outline-none focus:ring-2 focus:ring-focus bg-surface dark:bg-card transition-all placeholder:text-dim text-main rounded-xl"
                        />
                        {searchTerm && (
                            <button onClick={() => setSearchTerm('')} className="absolute start-2.5 top-1/2 -translate-y-1/2 text-dim hover:text-error transition-colors" aria-label="مسح البحث">
                                <X size={11} />
                            </button>
                        )}
                    </div>
                    {/* Day Filter */}
                    <div className="relative">
                        <Filter size={13} className="absolute end-3 top-1/2 -translate-y-1/2 text-dim" />
                        <select
                            value={filterDay}
                            onChange={(e) => setFilterDay(e.target.value)}
                            className="w-full pe-8 ps-3 py-2 border border-border text-micro font-bold outline-none focus:outline-none focus:ring-2 focus:ring-focus bg-surface dark:bg-card appearance-none cursor-pointer text-main transition-all rounded-xl"
                        >
                    ...
                    {/* Teacher Filter */}
                    <div className="relative">
                        <GraduationCap size={13} className="absolute end-3 top-1/2 -translate-y-1/2 text-dim" />
                        <select
                            value={filterTeacher}
