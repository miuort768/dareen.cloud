...
export const RecentActivity = ({ allPointLogs }: RecentActivityProps) => {
    ...
    return (
        <div className="bg-card dark:bg-card border border-border dark:border-primary/20 rounded-2xl p-5">
            <h3 className="text-[13px] font-bold text-main dark:text-main mb-3">آخر النشاطات</h3>
                        ...
                        <div key={log.id || `log-${i}`} className="flex items-start gap-3 relative">
                            ...
                            <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
                                <div className="min-w-0">
                                    ...
                                    {log.timestamp && (
                                        <p className="text-[10px] text-muted dark:text-dim flex items-center gap-1 mt-0.5">
            ...
            {recent.length === 0 && (
                <div className="py-8 text-center">
                    <Star size={24} className="mx-auto text-muted dark:text-dim mb-2" />
                    <p className="text-muted dark:text-dim font-medium text-[11px]">لا توجد نشاطات حديثة</p>
                </div>
            )}
        </div>
    );
