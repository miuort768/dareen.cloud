...
const statusConfig: Record<string, { label: string; dot: string; bg: string; text: string }> = {
  pending: { label: '»«‰ Ÿ«—', dot: 'bg-warning', bg: 'bg-warning-soft', text: 'text-warning' },
  completed: { label: ' „  »‰Ã«Õ', dot: 'bg-success', bg: 'bg-success-soft', text: 'text-success' },
  cancelled: { label: '„·€Ì…', dot: 'bg-error', bg: 'bg-error-soft', text: 'text-error' },
  converted: { label: '„ÕÊ·…', dot: 'bg-primary', bg: 'bg-primary-soft', text: 'text-primary' },
}
...
const formatPhone = (phone: string) => {
  if (!phone) return ''
  if (phone.length > 8) return `${phone.slice(0, 4)}ïïï${phone.slice(-3)}`
  return phone
}
      ...
      <div className="cursor-pointer p-4 pb-3" onClick={onCardClick}>
        ...
        <div className="mb-3 grid grid-cols-1 gap-2.5 sm:grid-cols-3">
          <div className="rounded-xl border border-primary-soft bg-primary-soft p-2.5 text-right">
            <p className="mb-1 text-[10px] font-bold text-primary">«·„«œ…</p>
          ...
          <div className="rounded-xl border border-warning-soft bg-warning-soft p-2.5 text-right">
            <p className="mb-1 text-[10px] font-bold text-warning">—ﬁ„ «· Ê«’·</p>
            <div className="flex items-center justify-start gap-1.5">
              <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-white/60 dark:bg-white/10">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          {onPaid && (
            <button
              ...
              aria-label="„œ›Ê⁄…"
            >
              <CircleDollarSign size={14} /> {isPaid ? ' „ «·œ›⁄' : 'œ›⁄'}
            </button>
          )}
        </div>
        <button
