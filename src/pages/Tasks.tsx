...
export const Tasks = () => {
    ...
    const deleteTask = async (id: string) => {
        if (!window.confirm('Â·  —Ìœ Õ–› Â–Â «·„Â„… ‰Â«∆Ì«ø')) return;
        try {
    ...
    return (
        <div className="min-h-full pb-24 overflow-x-hidden relative bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 dark:from-[#020617] dark:via-slate-950 dark:to-indigo-950/20 font-sans" dir="rtl">
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-indigo-900 via-indigo-800 to-slate-900 dark:from-slate-950 dark:via-indigo-950 dark:to-slate-950 rounded-none shadow-sm shadow-indigo-500/15 border border-white/5 px-6 md:px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                ...
                <div className="relative z-10 flex flex-col items-center">
                    ...
                    <p className="text-[11px] md:text-sm font-normal text-slate-400 uppercase tracking-widest mb-10 max-w-lg leading-relaxed">
                         Õ·Ì· „ ﬁœ„ Ê≈œ«—… –ﬂÌ… ·ﬂ«›… „Â«„ <span className="text-indigo-500">œ«—Ì‰ «·”«»⁄…</span>
                    </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-4">
                {[
                    { label: '„Â«„ „⁄·ﬁ…', value: stats.pending, icon: Clock, color: 'text-amber-500', bg: 'bg-amber-500/5', border: 'border-amber-500/20' },
                    { label: 'ﬁÌœ «· ‰›Ì–', value: stats.inProgress, icon: RefreshCcw, color: 'text-indigo-500', bg: 'bg-indigo-500/5', border: 'border-indigo-500/20' },
                    { label: '„⁄œ· «·≈‰Ã«“', value: `${stats.score}%`, icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-500/5', border: 'border-emerald-500/20' },
                    { label: '„Â«„ „ﬂ „·…', value: stats.completed, icon: CheckCircle2, color: 'text-purple-500', bg: 'bg-purple-500/5', border: 'border-purple-500/20' }
                ].map((stat, i) => (
            ...
            <div className="px-4 flex flex-col md:flex-row gap-4 items-center">
                ...
                <div className="flex gap-2 w-full md:w-auto overflow-x-auto no-scrollbar">
                    {['high', 'medium', 'low', 'all'].map(p => (
                        ...
                        >
                            {p === 'all' ? '«·ﬂ·' : p === 'high' ? '⁄«Ã· Ãœ«' : p === 'medium' ? '„ Ê”ÿ' : 'Â«œ∆'}
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="px-4 grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
                {filteredTasks.length > 0 ? (
                        ...
                        return (
                            ...
                            >
                                ...
                                <p className="text-slate-500 dark:text-slate-400 text-xs font-medium leading-loose mb-10 line-clamp-3">
                                    {task.description || "·«  ÊÃœ  ›«’Ì· ≈÷«›Ì… ·Â–« «·”Ã· «·⁄„·Ì« Ì..."}
                                </p>
                    ...
                    <div className="col-span-full py-20 text-center bg-white dark:bg-slate-900 border-2 border-dashed border-slate-100 dark:border-slate-800">
                        ...
                        <h2 className="text-xl font-medium text-slate-800 dark:text-white uppercase tracking-tighter mb-2">«·”Ã·«  ›«—€…</h2>
                        <p className="text-[10px] text-slate-400 font-normal uppercase tracking-widest">€—›… «·⁄„·Ì«  Ã«Â“… ·«” ﬁ»«· „Â«„ ÃœÌœ…</p>
                    </div>
                )}
            </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-none border-t-8 border-indigo-600 w-full max-w-xl shadow-[20px_20px_0px_rgba(79,70,229,0.1)] overflow-hidden">
                        <div className="p-8 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="text-lg font-medium text-slate-800 dark:text-white uppercase tracking-tighter">≈‰‘«¡ ⁄„·Ì… ÃœÌœ…</h3>
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-medium text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                        <Sparkles size={12} className="text-indigo-500" /> „”„Ï «·⁄„·Ì…
                                    </label>
                                    <input 
                                        required
                                        type="text" 
                                ...
                                <div className="grid grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">„” ÊÏ «·ŒÿÊ—…</label>
                                        <select 
                                        ...
                                        >
                                            <option value="low">Â«œ∆ (Low)</option>
                                            <option value="medium">„ Ê”ÿ (Medium)</option>
                                            <option value="high">⁄«Ã· (Critical)</option>
                                        </select>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-medium text-slate-500 uppercase tracking-widest"> «—ÌŒ «·«” Õﬁ«ﬁ</label>
                                        <input 
                                            type="date" 
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-medium text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                        <ShieldCheck size={12} className="text-indigo-500" />  ›«’Ì· «·»—Ê ÊﬂÊ·
                                    </label>
                                    <textarea 
