                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2 w-full md:w-auto">
                        <div className="relative flex-1 md:w-44">
                            <BookMarked className="absolute end-3 top-1/2 -translate-y-1/2 text-white/50" size={14} />
                            <select
                                value={subjectFilter}
                                onChange={e => setSubjectFilter(e.target.value)}
                                className="w-full bg-white/15 text-on-primary py-3 pe-10 ps-3 text-xs font-bold focus:outline-none border border-white/20 appearance-none cursor-pointer"
                            >
                        ...
                        <div className="relative flex-1 md:w-48">
                            <Search className="absolute end-4 top-1/2 -translate-y-1/2 text-white/50" size={16} />
                            <input
                                type="text"
                                placeholder="بحث..."
                                value={search}
                                onChange={e => setSearch(e.target.value)}
                                className="w-full bg-white/15 text-on-primary placeholder:text-white/50 py-3 pe-12 ps-4 text-xs font-bold focus:outline-none border border-white/20"
                            />
                        </div>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    filtered.map(app => (
                        <div key={app.id} className={`bg-white border border-border/50 shadow-sm relative overflow-hidden group transition-all duration-300 ${
                            ...
                            {/* Decorative pattern */}
                            {!app.contacted && <div className="absolute top-0 start-0 w-24 h-24 bg-primary/5 -me-6 -mt-6 rotate-45 pointer-events-none border border-primary/10"></div>}
                            ...
                            <div className={`p-6 relative z-10 transition-all duration-300 ${
                                ...
                                <div className={`flex items-center gap-4 mb-6 p-4 rounded-lg ${
                                    ...
                                    <div className="w-12 h-14 bg-white/20 border-2 border-white/30 flex items-center justify-center relative overflow-hidden shrink-0">
                                        ...
                                        <div className="absolute bottom-0 start-0 w-full h-0.5 bg-white/50"></div>
                                    </div>
