...
export const SectionTitle = ({ icon: Icon, label, sub }: { icon: React.ComponentType<{ size?: number }>; label: string; sub?: string }) => (
  <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-100/50 dark:border-slate-800/50">
    <div className="w-8 h-8 rounded-none flex items-center justify-center" style={{ backgroundColor: '#8B5CF612' }}>
...
export const SecondaryBtn = ({ onClick, children, className = '', title }: {
  ...
  <button
    ...
    className={cn(
      ...
      'text-xs font-bold px-3 py-2 border border-slate-200 dark:border-slate-700 transition-all shadow-sm rounded-none',
      'active:scale-[0.97]',
      className
    )}
  >
...
export const DangerBtn = ({ onClick, children, className = '', title }: {
  ...
  <button
    ...
    className={cn(
      ...
      'text-xs font-bold px-3 py-2 transition-all shadow-sm rounded-none',
      'active:scale-[0.97]',
      className
    )}
  >
