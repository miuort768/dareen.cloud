...
const inputClass = "w-full bg-white/[0.04] border border-white/[0.08] px-3.5 py-3 text-[13px] outline-none focus:border-[#6366f1]/50 focus:ring-2 focus:ring-[#6366f1]/10 text-white rounded-xl transition-all duration-200 placeholder:text-white/20 font-bold";
            ...
            {isAddModalOpen && (
                <>
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm" onClick={() => setIsAddModalOpen(false)} />
                    <motion.div
                        initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }}
                        transition={{ type: 'spring', damping: 35, stiffness: 300 }}
                        className="fixed inset-x-0 bottom-0 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 z-[210] sm:w-full sm:max-w-lg max-h-[90vh] sm:max-h-[85vh] bg-[#0a0e27] sm:border sm:border-white/[0.06] sm:shadow-2xl sm:rounded-2xl flex flex-col overflow-hidden"
                        dir="rtl"
                    >
                        ...
                        <div className="shrink-0 px-5 py-4 flex items-center justify-between relative overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-l from-[#6366f1]/10 to-[#8b5cf6]/5" />
                            <div className="relative z-10 flex items-center gap-3">
                                <div className="w-10 h-10 rounded-2xl flex items-center justify-center bg-[#6366f1]/15">
                                    <UserPlus size={18} className="text-[#a5b4fc]" />
                                </div>
                                <div>
                                    <h2 className="text-sm font-bold text-white">إضافة عميل جديد</h2>
                                    <p className="text-[10px] text-white/30 mt-0.5">أدخل بيانات العميل الجديد</p>
                                </div>
                            </div>
                            <button onClick={() => setIsAddModalOpen(false)} className="relative z-10 w-8 h-8 flex items-center justify-center bg-white/5 hover:bg-white/10 rounded-xl transition-all" aria-label="إغلاق">
                        ...
                        <form ref={formRef} className="flex-1 overflow-y-auto p-5 space-y-4" onSubmit={(e) => {
                            ...
                            <div className="flex gap-3 pt-2 pb-4">
                                ...
                                <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-3.5 text-[11px] font-bold text-white/40 bg-white/5 hover:bg-white/10 rounded-xl transition-all active:scale-[0.98]">إلغاء</button>
                            </div>
                        </form>
                    </motion.div>
                </>
