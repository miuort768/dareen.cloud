            ...
            {filteredLeads.length === 0 ? (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="py-20 text-center">
                    <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-surface dark:bg-white/5 flex items-center justify-center">
                        <Search size={28} className="text-muted/30 dark:text-main/20" />
                    </div>
                    <p className="text-sm font-bold text-muted dark:text-main/60 mb-1">لا توجد نتائج</p>
                    <p className="text-xs text-muted/60 dark:text-main/30">لا يوجد عملاء متطابقون مع معايير البحث</p>
                </motion.div>
            ) : (
                    ...
                    {filteredLeads.map((lead, idx) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className="bg-card dark:bg-card/80 border border-border dark:border-white/[0.06] rounded-2xl active:scale-[0.98] transition-all duration-200 cursor-pointer overflow-hidden shadow-sm dark:shadow-none"
                            >
                                {/* Header */}
                                    ...
                                    <div className="flex items-center gap-3 min-w-0 flex-1">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2">
                                                <h4 className="font-bold text-[13px] text-main dark:text-main truncate">{lead.studentName || 'عميل بدون اسم'}</h4>
                                                <StatusChip status={lead.status as LeadStatus} size="sm" />
                                            </div>
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="text-[11px] text-muted dark:text-main/40 font-mono">{lead.phone}</span>
                                                <span className="text-[11px] text-border dark:text-main/20">•</span>
                                                <span className="flex items-center gap-1 text-[10px] text-muted/60 dark:text-main/30">
                                                    <Clock size={8} />
                                                    {age.text}
                                                </span>
                                            </div>
                                ...
                                <div className="px-4 pb-2 flex items-center gap-1.5 flex-wrap">
                                    {lead.subject && (
                                        <span className="text-[10px] text-muted dark:text-main/50 bg-surface dark:bg-white/5 border border-border dark:border-white/[0.06] px-2 py-0.5 rounded-full">{lead.subject}</span>
                                    )}
                                    {lead.curriculum && (
                                        <span className="text-[10px] text-muted dark:text-main/50 bg-surface dark:bg-white/5 border border-border dark:border-white/[0.06] px-2 py-0.5 rounded-full">{lead.curriculum}</span>
                                    )}
                                    {lead.source && (
                                        <span className="text-[10px] text-muted dark:text-main/50 bg-surface dark:bg-white/5 border border-border dark:border-white/[0.06] px-2 py-0.5 rounded-full">{lead.source}</span>
                                    )}
                                </div>
                                ...
                                {lead.notes && (
                                    <div className="mx-4 mb-3 bg-warning/5 dark:bg-white/[0.03] border border-warning/10 dark:border-white/[0.04] px-3 py-2 rounded-xl">
                                        <p className="text-[11px] text-muted dark:text-main/40 leading-relaxed line-clamp-2">{lead.notes}</p>
                                    </div>
                                )}
