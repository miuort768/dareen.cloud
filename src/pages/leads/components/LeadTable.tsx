        ...
        return (
            <div className="hidden lg:block">
                <div className="py-20 text-center">
                    <Search size={48} className="mx-auto mb-4 text-muted/20 dark:text-white/10" />
                    <p className="text-sm font-bold text-muted dark:text-white/40">لا توجد نتائج</p>
                    <p className="text-xs text-muted/60 dark:text-white/20 mt-1">لا يوجد عملاء متطابقون مع معايير البحث</p>
                </div>
            </div>
        );
    }
            ...
            <Virtuoso
                ...
                fixedHeaderContent={() => (
                    <div className="flex items-center px-5 py-3 bg-surface/80 dark:bg-white/[0.02] backdrop-blur-sm border-b border-border dark:border-white/[0.04]">
                        <div className="w-[22%] px-2 font-bold text-[11px] tracking-wide text-muted/50 dark:text-white/30 text-end">العميل</div>
                        <div className="w-[13%] px-2 font-bold text-[11px] tracking-wide text-muted/50 dark:text-white/30 text-end">التواصل</div>
                        <div className="w-[13%] px-2 font-bold text-[11px] tracking-wide text-muted/50 dark:text-white/30 text-end">المادة</div>
                        <div className="w-[14%] px-2 font-bold text-[11px] tracking-wide text-muted/50 dark:text-white/30 text-end">الحالة</div>
                        <div className="w-[10%] px-1 font-bold text-[11px] tracking-wide text-muted/50 dark:text-white/30 text-center">الأولوية</div>
                        <div className="w-[28%] px-2 font-bold text-[11px] tracking-wide text-muted/50 dark:text-white/30 text-center">الإجراءات</div>
                    </div>
                )}
                        ...
                        <div onClick={() => onLeadClick(lead)}
                            ...
                            <div className="w-[13%] px-2">
                                <span onClick={(e) => { e.stopPropagation(); window.open(`https://wa.me/${lead.phone}`, '_blank'); }}
                                    className="font-mono text-xs text-main/60 dark:text-white/60 hover:text-success dark:hover:text-[#34d399] cursor-pointer transition-colors">{lead.phone}</span>
                            </div>
                            <div className="w-[13%] px-2">
                                <span className="text-[11px] text-muted bg-surface dark:text-white/40 dark:bg-white/[0.04] border border-border dark:border-white/[0.06] px-2 py-1 rounded-lg">{lead.subject}</span>
                            </div>
                            <div className="w-[14%] px-2">
                                <select
                                    className="px-2.5 py-1.5 text-[10px] font-bold border border-border dark:border-white/[0.08] outline-none cursor-pointer rounded-lg bg-surface dark:bg-white/[0.04] transition-all text-main dark:text-white/60"
                                    value={lead.status}
