            ...
            >
                ...
                <div className="bg-gradient-to-l from-error to-error-hover px-5 py-5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-sm font-bold text-on-error"> √ﬂÌœ «·Õ–›</h3>
                            <p className="text-[10px] text-on-error/70 mt-0.5">·« Ì„ﬂ‰ «· —«Ã⁄ ⁄‰ Â–« «·≈Ã—«¡</p>
                        </div>
                    </div>
                    <button onClick={onCancel} className="w-8 h-8 flex items-center justify-center bg-white/15 hover:bg-white/25 text-on-error rounded-xl transition-all" aria-label="≈€·«ﬁ"><X size={14} /></button>
                </div>
                <div className="p-5">
                    <p className="text-sm font-bold text-main mb-1">Â· √‰  „ √ﬂœ „‰ Õ–› Â–« «·⁄„Ì·ø</p>
                    <p className="text-xs text-muted leading-relaxed">”Ì „ ‰ﬁ· «·⁄„Ì· <span className="text-error font-bold">«·„›ﬁÊœ</span> ≈·Ï ﬁ«∆„… «·⁄„·«¡ «·„›ﬁÊœÌ‰ Ê·‰ ÌŸÂ— „—… √Œ—Ï.</p>
                </div>
                <div className="flex gap-2 p-5 pt-0">
                    <button ref={cancelRef} type="button" onClick={onCancel} className="flex-1 py-3.5 text-xs font-bold text-muted bg-surface hover:bg-hover rounded-xl transition-all active:scale-[0.98]">≈·€«¡</button>
                    <button onClick={onConfirm} className="flex-1 py-3.5 text-xs font-bold text-on-error bg-gradient-to-l from-error to-error-hover hover:from-error-hover hover:to-error rounded-xl transition-all active:scale-[0.98] shadow-lg shadow-error/20"> √ﬂÌœ «·Õ–›</button>
                </div>
            </motion.div>
        </motion.div>
    );
        ...
        <div className="bg-card dark:bg-card/80 border border-border dark:border-white/[0.04] rounded-2xl overflow-hidden">
            <div className="px-5 py-4 flex items-center justify-between border-b border-border/50 dark:border-white/[0.04]">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[13px] font-bold text-main dark:text-main">≈÷«›… ⁄„Ì· ÃœÌœ</h2>
                        <p className="text-[10px] text-muted/60 dark:text-main/30">√œŒ· »Ì«‰«  «·⁄„Ì·</p>
                    </div>
                </div>
                <button onClick={onClose} className="w-8 h-8 flex items-center justify-center bg-surface dark:bg-white/5 hover:bg-hover dark:hover:bg-white/10 rounded-xl transition-all" aria-label="≈€·«ﬁ">
            ...
            <form ref={formRef} className="p-5 space-y-3" onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); const g = (n: string) => (fd.get(n) as string) || ''; addMutation.mutate({ studentName: g('name'), phone: g('phone'), subject: g('subject'), curriculum: g('curriculum'), status: 'new', priority: g('priority'), notes: g('notes') }); }}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div><label className={labelClass}>«”„ «·ÿ«·» («Œ Ì«—Ì)</label><input name="name" className={inputClass} placeholder="„À«·: √„ √Õ„œ" /></div>
                    <div><label className={labelClass}>«·„‰ÂÃ</label><input name="curriculum" required className={inputClass} placeholder="„À«·: „’—Ì" /></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div><label className={labelClass}>—ﬁ„ «·Â« ›</label><input name="phone" required className={inputClass} placeholder="05XXXXXXXX" dir="ltr" style={{ textAlign: 'right' }} /></div>
                    <div><label className={labelClass}>«·„«œ… «·„Â „ »Â«</label><input name="subject" required className={inputClass} placeholder="„À«·: —Ì«÷Ì« " /></div>
                </div>
                <div><label className={labelClass}>«·√Ê·ÊÌ…</label><select name="priority" aria-label="«·√Ê·ÊÌ…" className={inputClass}><option value="low">„‰Œ›÷…</option><option value="medium">„ Ê”ÿ…</option><option value="high">⁄«·Ì…</option></select></div>
                <div><label className={labelClass}>„·«ÕŸ« </label><textarea name="notes" rows={2} className={inputClass + " resize-none"} placeholder="«ﬂ » √Ì  ›«’Ì·..." /></div>
                <div className="flex gap-3 pt-1">
                    <PrimaryBtn type="submit" disabled={addMutation.isPending} className="flex-1 py-3">{addMutation.isPending ? 'Ã«—Ì «·Õ›Ÿ...' : '≈÷«›… «·⁄„Ì·'}</PrimaryBtn>
                    <button type="button" onClick={onClose} className="flex-1 py-3 text-[11px] font-bold text-muted dark:text-main/40 bg-surface dark:bg-white/5 hover:bg-hover dark:hover:bg-white/10 rounded-xl transition-all">≈·€«¡</button>
                </div>
            </form>
        </div>
    </motion.div>
...
export const Leads = () => {
    ...
    useEffect(() => { document.title = `«·⁄„·«¡ «·„Õ „·Ê‰ | ${academyName} ·· ⁄·Ì„ Ê«· œ—Ì»`; }, [academyName]);
    ...
    const addMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            showNotification(' „  ≈÷«›… «·⁄„Ì· »‰Ã«Õ', 'success');
        },
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['leads'] });
            queryClient.invalidateQueries({ queryKey: ['lead-stats'] });
            showNotification(' „  ÕœÌÀ «·⁄„Ì· »‰Ã«Õ', 'success');
        },
        ...
        >
            <div className="relative z-10 mx-auto px-4 md:px-6 max-w-page">
                    ...
                    <div className="relative z-10 px-5 md:px-8 py-5 md:py-8">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
                                <p className="text-white/60 dark:text-main/40 text-[11px] mb-1">„—Õ»« »ﬂ! ??</p>
                                <h1 className="text-lg md:text-2xl font-bold font-outfit text-white mb-1 tracking-tight">≈œ«—… ⁄„·«∆ﬂ »”ÂÊ·…</h1>
                                <p className="text-white/50 dark:text-main/40 text-[11px] md:text-sm"> «»⁄ Ê√œ— Ã„Ì⁄ «·⁄„·«¡ «·„ Êﬁ⁄Ì‰ ÊÕÊ¯·Â„ ≈·Ï ⁄ﬁÊœ ‰«ÃÕ…</p>
                            </motion.div>
                            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }} className="flex items-center gap-2">
                                ...
                                >
                                    ...
                                    <span className="hidden sm:inline">{showLost ? '«·‰‘ÿ«¡' : '«·„›ﬁÊœÌ‰'}</span>
                                </button>
                        ...
                        <div className="grid grid-cols-4 gap-1.5 sm:gap-2.5 md:gap-3">
                            {[
                                { label: '≈Ã„«·Ì «·⁄„·«¡', value: stats?.total || 0, sub: `${activeCount} ‰‘ÿ`, icon: Users, delay: 0.15 },
                                { label: '⁄„·«¡ Ãœœ', value: stats?.new || 0, sub: 'Â–« «·‘Â—', icon: Activity, delay: 0.2, accent: true },
                                { label: ' „ «· ÕÊÌ·', value: stats?.converted || 0, sub: '≈·Ï „‘ —ﬂÌ‰', icon: Phone, delay: 0.25 },
                                { label: '„⁄œ· «· ÕÊÌ·', value: `${(stats?.conversionRate ?? 0).toFixed(1)}%`, sub: '„⁄œ· «·‰Ã«Õ', icon: BarChart3, delay: 0.3 },
                            ].map((stat, i) => (
                    ...
                    <div className="p-4 lg:p-5 border-b border-border dark:border-white/[0.04]">
                        <div className="flex items-center gap-3">
                            <div className="relative flex-1">
                                ...
                                />
                                {searchTerm && (
                                    <button aria-label="„”Õ «·»ÕÀ" onClick={() => setSearchTerm('')} className="absolute left-3 top-1/2 -translate-y-1/2 w-6 h-6 flex items-center justify-center text-muted dark:text-main/25 hover:text-main dark:hover:text-white/60 rounded-lg transition-all">
                                        <X size={13} />
                                    </button>
                                )}
                            </div>
                ...
                <motion.button
                    ...
                    whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} aria-label="≈÷«›… ⁄„Ì·">
                    <Plus size={22} />
                </motion.button>
