        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-primary-active w-full max-w-sm shadow-xl rounded-2xl overflow-hidden border border-border dark:border-border">
            <div className="bg-gradient-to-br from-[var(--bg-error)] to-[var(--bg-error)] px-5 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-bold text-on-primary">Õ–› «·⁄„Ì·</h3>
                </div>
                <button onClick={onCancel} className="w-7 h-7 flex items-center justify-center hover:bg-white/10 text-on-primary/70 rounded-xl transition-all" aria-label="≈·€«¡"><X size={16} /></button>
            </div>
            <div className="p-5">
                <p className="text-sm font-bold text-main dark:text-dim mb-1">Â· √‰  „ √ﬂœ „‰ Õ–› Â–« «·⁄„Ì·ø</p>
                <p className="text-xs font-bold text-muted leading-relaxed">
                    ”Ì „ Õ–› «·⁄„Ì· <span className="text-error font-bold">‰Â«∆Ì«</span> „‰ ﬁ«⁄œ… «·»Ì«‰«  Ê·« Ì„ﬂ‰ «· —«Ã⁄.
                </p>
            </div>
            <div className="flex gap-2 p-5 pt-0">
                <button onClick={onCancel} className="flex-1 py-3 text-xs font-bold text-muted bg-surface dark:bg-primary-active hover:bg-surface dark:hover:bg-primary-active rounded-xl transition-all active:scale-[0.98]">≈·€«¡</button>
                <button onClick={onConfirm} className="flex-1 py-3 text-xs font-bold text-on-primary bg-gradient-to-l from-[var(--bg-error)] to-[var(--bg-error)] hover:from--[var(--bg-error)] hover:to--[var(--bg-error)] rounded-xl transition-all active:scale-[0.98] shadow-sm shadow-error/20"> √ﬂÌœ «·Õ–›</button>
            </div>
        </motion.div>
    </motion.div>
);
...
export const Leads: React.FC = () => {
    ...
    const addMutation = useMutation({
        ...
        onError: (err: Error & { response?: { data?: { error?: string } } }) => { alert('ÕœÀ Œÿ√ √À‰«¡ «·≈÷«›…: ' + (err?.response?.data?.error || err.message)); }
    });
    ...
    const statusConfig: Record<LeadStatus, { label: string, color: string, bg: string }> = {
        new: { label: 'ÃœÌœ', color: 'text-info', bg: 'bg-info-light dark:bg-info/20' },
        contacted: { label: ' „ «· Ê«’·', color: 'text-warning', bg: 'bg-warning-light dark:bg-warning/20' },
        interested: { label: '„Â „', color: 'text-success', bg: 'bg-success-light dark:bg-success/20' },
        trial: { label: 'Õ’…  Ã—Ì»Ì…', color: 'text-primary', bg: 'bg-primary-soft dark:bg-primary-active/20' },
        converted: { label: '„‘ —ﬂ', color: 'text-info', bg: 'bg-info-light dark:bg-info/20' },
        lost: { label: '„·€Ì', color: 'text-error', bg: 'bg-error-light dark:bg-error/20' }
    };
            ...
            <div className="relative z-10 mx-auto px-2 md:px-4 max-w-7xl">
                ...
                <div className="bg-gradient-to-br from-[var(--bg-primary)] to-[var(--bg-primary)] rounded-2xl px-5 md:px-7 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 shadow-lg shadow-primary/20 mt-6 md:mt-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-lg font-bold text-on-primary leading-tight">«·⁄„·«¡ «·„Õ „·Ì‰</h1>
                            <p className="text-[10px] font-bold text-on-primary/70 mt-0.5">≈œ«—… ÿ·»«  «· ”ÃÌ· Ê«·„Â „Ì‰</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                    <StatItem title="≈Ã„«·Ì «·„Â „Ì‰" value={stats?.total || 0} icon={Users} />
                    <StatItem title="⁄„·«¡ Ãœœ" value={stats?.new || 0} icon={Clock} />
                    <StatItem title=" „ «· ÕÊÌ·" value={stats?.converted || 0} icon={CheckCircle2} />
                    <StatItem title="„⁄œ· «· ÕÊÌ·" value={`${(stats?.conversionRate ?? 0).toFixed(1)}%`} icon={TrendingUp} />
                </div>
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto">
                            ...
                            <div className="relative w-full md:w-auto">
                                <select className="w-full md:w-auto appearance-none bg-background dark:bg-primary-active border border-border dark:border-border rounded-xl px-3 py-2 text-[11px] font-bold outline-none cursor-pointer focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all text-main dark:text-on-primary" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as LeadStatus | 'all')}>
                                    <option value="all" className="text-main">ﬂ· «·Õ«·« </option>
