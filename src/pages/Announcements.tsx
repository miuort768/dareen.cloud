    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingAnnouncement) {
                ...
                showNotification(' „  ÕœÌÀ «·≈⁄·«‰ »‰Ã«Õ', 'success');
            } else {
                await api.post('/announcements', payload);
                showNotification(' „ ‰‘— «·≈⁄·«‰ »‰Ã«Õ', 'success');
            }
            ...
            showNotification('›‘· Õ›Ÿ «·≈⁄·«‰', 'error');
        }
    };

    const handleDelete = async (id: string) => {
        if (!window.confirm('Â· √‰  „ √ﬂœ „‰ Õ–› Â–« «·≈⁄·«‰ø')) return;
        try {
            await api.delete(`/announcements/${id}`);
            showNotification(' „ Õ–› «·≈⁄·«‰', 'success');
            fetchAnnouncements();
        } catch {
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-3 md:gap-5">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <span className="px-2 py-0.5 bg-white/10  text-white text-[8px] font-normal uppercase tracking-widest border border-white/10 italic leading-none">ÊÕœ… «· Õﬂ„ «·„—ﬂ“Ì…</span>
                                <div className="flex gap-1">
                                    {[1,2,3].map(i => <div key={i} className="w-1 h-1 bg-emerald-500 animate-pulse" style={{ animationDelay: `${i*0.2}s` }} />)}
                                </div>
                            </div>
                            <h1 className="text-base md:text-2xl font-medium text-white italic tracking-tight uppercase leading-none">«·‰‘—… Ê«· ⁄„Ì„« </h1>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        <div className="bg-white/5 border border-white/10 px-3 py-2 flex items-center gap-2  shrink-0">
                            ...
                            <span className="text-[6px] md:text-[7px] font-medium text-slate-400 uppercase tracking-widest leading-none border-r border-white/10 pr-2 md:pr-3">‰‘ÿ…<br/>»«·„‰’‹…</span>
                        </div>
                        <button
                            onClick={() => {
                                setEditingAnnouncement(null);
                        ...
                        >
                            ...
                            <span className="text-[8px] md:text-[10px] uppercase tracking-widest font-medium">≈’œ«—  ⁄„Ì„</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                ...
                {announcements.length === 0 && !isLoading && (
                    <div className="col-span-full py-20 bg-slate-50 dark:bg-slate-900/50 border border-dashed border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center opacity-30 text-center">
                        ...
                        <h3 className="text-lg font-medium uppercase italic tracking-widest text-slate-500">”Ã· «·≈⁄·«‰«  ›«—€</h3>
                    </div>
                )}
            </div>
                    ...
                    <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 border border-white/10 shadow-sm overflow-hidden rounded-none">
                        <div className="p-5 bg-slate-900 text-white flex items-center justify-between border-b border-white/5">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="font-medium text-xs uppercase italic tracking-tight">
                                    {editingAnnouncement ? ' ÕœÌÀ «·»Ì«‰«  «·„—ﬂ“Ì…' : '≈’œ«—  ⁄„Ì„ ≈œ«—Ì ÃœÌœ'}
                                </h3>
                            </div>
                        ...
                        <form onSubmit={handleSave} className="p-6 md:p-8 space-y-6">
                            <div className="space-y-1.5">
                                <label className="text-[9px] font-medium text-slate-500 uppercase tracking-widest italic leading-none mb-1">«”„ «· ⁄„Ì„ / «·⁄‰Ê«‰</label>
                                <input
                                    required
                                    type="text"
                            ...
                            <div className="grid grid-cols-2 gap-5">
                                <div className="space-y-1.5">
                                    <label className="text-[9px] font-medium text-slate-500 uppercase tracking-widest italic leading-none mb-1"> ’‰Ì› «·»Ì«‰« </label>
                                    <select
                                        value={formData.type}
                                    ...
                                    >
                                        ...
                                        <option value="urgent"> ‰»Ì‹Â ⁄‹«Ã·</option>
                                        <option value="holiday">≈Ã‹«“… —”„Ì‹…</option>
                                        <option value="event">›⁄«·Ì‹… ÃœÌ‹œ…</option>
                                    </select>
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[9px] font-medium text-slate-500 uppercase tracking-widest italic leading-none mb-1">Õ«·… «·‰‘—</label>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[9px] font-medium text-slate-500 uppercase tracking-widest italic leading-none mb-1">„Õ ÊÏ «· ⁄„Ì„</label>
                                <textarea
                                    required
                                    rows={4}
