...
export const Announcements = () => {
    ...
    const kpiCards = useMemo(() => [
        ...
        { label: 'النشطة', value: announcements.filter(a => a.isActive).length, icon: Bell, gradient: 'from-success-soft to-background dark:from-success-soft dark:to-card', iconBg: 'bg-white/50 text-success dark:bg-white/10', accent: 'bg-success' },
        { label: 'غير النشطة', value: announcements.filter(a => !a.isActive).length, icon: Filter, gradient: 'from-warning-soft to-background dark:from-warning-soft dark:to-card', iconBg: 'bg-white/50 text-warning dark:bg-white/10', accent: 'bg-warning' },
        { label: 'الأحداث', value: announcements.filter(a => a.type === 'event' || a.type === 'holiday').length, icon: Calendar, gradient: 'from-info-soft to-background dark:from-info-soft dark:to-card', iconBg: 'bg-white/50 text-info dark:bg-white/10', accent: 'bg-info' },
    ], [announcements]);
        ...
        <>
            ...
            <div className="min-h-full pb-24 overflow-x-hidden relative" dir="rtl">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary via-primary-deep to-primary-hover dark:from-primary dark:via-primary-deep dark:to-primary-hover border border-transparent dark:border-primary/40 p-6 md:p-8 mb-4">
                    {particles.map(p => (
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-white/70 text-xs font-medium">
                                    {isAdmin ? 'الإدارة' : currentUser?.role === 'teacher' ? 'للمعلمات' : 'لأولياء الأمور والطلاب'}
                                </span>
                            </div>
            ...
            {isAdmin && (
                <div className="fixed bottom-6 end-6 z-50 flex flex-col items-end gap-3">
                    ...
                    <motion.button onClick={() => setFabOpen(!fabOpen)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                        className={cn("w-12 h-12 rounded-2xl shadow-xl text-on-primary flex items-center justify-center transition-all", fabOpen ? "bg-error rotate-45" : "bg-primary")}>
                        <Plus size={24} />
                    </motion.button>
                </div>
            )}
