                ...
                <form onSubmit={onSubmit} className="p-6 space-y-5">
                    <div>
                        ...
                        <input required type="text" value={formData.title} onChange={(e) => onChange({ title: e.target.value })}
                            className="w-full px-4 py-3 bg-surface border border-border font-bold text-xs rounded-none outline-none focus:outline-none focus:ring-2 focus:ring-focus text-main transition-all placeholder:text-muted"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select value={formData.type} onChange={(e) => onChange({ type: e.target.value as AnnouncementType })}
                                ...
                                className="w-full px-4 py-3 bg-surface border border-border font-bold text-xs rounded-none outline-none focus:outline-none focus:ring-2 focus:ring-focus text-main transition-all cursor-pointer dark:[color-scheme:dark]">
                        ...
                        <div>
                            ...
                            <div className="flex gap-1 h-11">
                                <button type="button" onClick={() => onChange({ isActive: true })}
                                    className={cn("flex-1 font-bold text-micro rounded-none transition-all active:scale-95", formData.isActive ? "bg-success text-on-success" : "bg-surface text-muted")}>
                                    نشط
                                </button>
                                <button type="button" onClick={() => onChange({ isActive: false })}
                                    className={cn("flex-1 font-bold text-micro rounded-none transition-all active:scale-95", !formData.isActive ? "bg-warning text-on-warning" : "bg-surface text-muted")}>
                                    مخفي
                                </button>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea required rows={4} value={formData.content} onChange={(e) => onChange({ content: e.target.value })}
                            className="w-full px-4 py-3 bg-surface border border-border font-bold text-xs rounded-none outline-none focus:outline-none focus:ring-2 focus:ring-focus text-main transition-all leading-relaxed resize-none placeholder:text-muted"
                            placeholder="أكتب محتوى الإعلان هنا..." />
                    </div>
                    <button type="submit"
                        className="w-full py-3.5 bg-primary text-on-primary font-bold text-xs hover:bg-primary-hover transition-all flex items-center justify-center gap-2 active:scale-95 rounded-none">
