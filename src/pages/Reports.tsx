...
import { Award, CheckCircle2, DollarSign, LayoutDashboard, Users, TrendingUp, Calendar, BarChart3, Plus, FileText } from 'lucide-react';
...
export const Reports = () => {
    ...
    const kpiCards = useMemo(() => [
        ...
        { label: 'الاشتراكات', value: state.totalEnrollments, icon: Award, gradient: 'from-success/20 to-success/5', iconBg: 'bg-success/10 text-success', accent: 'bg-success' },
        { label: 'الإيرادات', value: (state.totalRevenue > 1000 ? Math.round(state.totalRevenue / 1000) + 'k' : state.totalRevenue) + ' ' + CURRENCY_SYMBOL, icon: DollarSign, gradient: 'from-warning/20 to-warning/5', iconBg: 'bg-warning/10 text-warning', accent: 'bg-warning' },
        ...
        <div className="min-h-full pb-24 overflow-x-hidden relative" dir="rtl">
            <div className="max-w-page mx-auto px-2 space-y-4">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-white/70 text-sm">تحليل الأداء الأكاديمي والمالي المتكامل</p>
                        </div>
                        <div className="flex items-center gap-4 bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-2xl font-bold text-white">{state.revenueGrowth >= 0 ? `+${state.revenueGrowth}%` : `${state.revenueGrowth}%`}</p>
                            </div>
                        </div>
                    </div>
                </motion.div>
                ...
                <div className="no-print bg-surface border border-border rounded-2xl p-1 flex overflow-x-auto no-scrollbar gap-1">
                    {tabs.map((tab) => {
                        const Icon = tab.icon;
                        const isActive = state.activeReport === tab.id || (state.activeReport === 'enrollment' && tab.id === 'academic');
                        return (
                            <button key={tab.id} onClick={() => actions.setActiveReport(tab.id as ReportType)}
                                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${isActive ? 'bg-primary text-on-primary shadow-xs' : 'text-muted hover:text-main'}`}>
                        ...
                        <div className="space-y-4">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
                                    <div>
                                        ...
                                        <p className="text-xs font-bold text-muted leading-relaxed max-w-md">
                                            تقرير شامل يوضح الحالة الأكاديمية والمالية للمؤسسة. تم تحليل {state.totalEnrollments} اشتراك نشط عبر {uniqueSubjects} مادة دراسية.
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-4 p-4 rounded-xl border bg-surface border-border">
                                        ...
                                        <div><p className="text-xs font-bold text-success">النمو الشهري</p><p className="text-2xl font-bold font-mono leading-none mt-1 text-main">{state.revenueGrowth >= 0 ? `+${state.revenueGrowth}%` : `${state.revenueGrowth}%`}</p></div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                {tabs.filter(t => t.id !== 'overview').map((tab) => {
                                    ...
                                    return (
                                        <button key={tab.id} onClick={() => actions.setActiveReport(tab.id as ReportType)}
                                            className={`border-0 p-4 transition-all group rounded-2xl hover:shadow-md active:scale-95 ${v.bg}`}>
                                            <div className="flex flex-col items-center text-center">
                                                ...
                                                <p className={`text-micro font-bold mt-1 ${v.text} opacity-60`}>عرض التقرير التفصيلي</p>
                                            </div>
                                        </button>
                                    );
                                })}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {[
                                    ...
                                    { label: 'الاشتراكات', value: state.totalEnrollments, icon: Award, textClass: 'text-chart-1', bgClass: 'bg-chart-1/10' },
                                    { label: 'المواد', value: uniqueSubjects, icon: BarChart3, textClass: 'text-chart-4', bgClass: 'bg-chart-4/10' },
                                    { label: 'الحصص الكلية', value: state.totalSessions, icon: Calendar, textClass: 'text-chart-2', bgClass: 'bg-chart-2/10' },
                                    { label: 'الحصص المكتملة', value: state.completedSessions, icon: CheckCircle2, textClass: 'text-chart-6', bgClass: 'bg-chart-6/10' },
                                    { label: 'الإيرادات', value: (state.totalRevenue > 1000 ? Math.round(state.totalRevenue / 1000) + 'k' : state.totalRevenue) + ' ' + CURRENCY_SYMBOL, icon: DollarSign, textClass: 'text-chart-5', bgClass: 'bg-chart-5/10' },
                                    { label: 'نسبة الحضور', value: state.attendanceRate + '%', icon: TrendingUp, textClass: 'text-chart-3', bgClass: 'bg-chart-3/10' },
                                    { label: 'النمو الشهري', value: (state.revenueGrowth >= 0 ? `+${state.revenueGrowth}%` : `${state.revenueGrowth}%`), icon: TrendingUp, textClass: 'text-primary', bgClass: 'bg-primary-soft' }
                                ].map((stat, i) => (
                                    <div key={`stat-${i}`} className="bg-card border border-border rounded-2xl p-4 flex flex-col justify-between">
                                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${stat.bgClass}`}>
                                            <stat.icon size={15} className={stat.textClass} />
                                        </div>
                                        <div className="mt-3">
                                            <p className={`text-base font-bold font-mono leading-none ${stat.textClass}`}>{stat.value}</p>
                                            <p className="text-xs font-bold mt-1.5 truncate text-muted">{stat.label}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="bg-card border border-border rounded-2xl p-5">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <span className="text-micro font-bold px-2.5 py-1 rounded-xl bg-success-soft text-success">تحليل مباشر</span>
                                </div>
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                                    {state.subjectPieData.slice(0, 6).map((s, i) => {
                                        ...
                                        return (
                                            <div key={`report-${i}`} className="flex flex-col gap-2 p-3.5 rounded-2xl border transition-all bg-card" style={{ borderColor: `color-mix(in srgb, ${cc.var} 25%, transparent)` }}>
                                                <div className="flex items-center justify-between">
                                                    <div className={`w-2.5 h-2.5 rounded-sm ${cc.bg}`} />
                                                    <p className={`text-micro font-bold font-mono ${cc.text}`}>{pct}%</p>
                                                </div>
                                                <p className="text-xs font-bold text-main truncate">{s.name}</p>
                                                <div className="w-full h-1.5 rounded-full overflow-hidden bg-surface">
                                                    <div className={`h-full rounded-full transition-all ${cc.bg}`} style={{ width: `${pct}%` }} />
                                                </div>
                                            </div>
                                        );
                                    })}
