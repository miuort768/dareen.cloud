                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    "flex items-center gap-2 px-3 py-3 rounded-xl transition-all border text-right",
                                    isActive
            ...
            <div className="px-0 transition-all duration-300 animate-in fade-in slide-in-from-bottom-2">
                ...
                {state.activeReport === 'overview' && (
                    <div className="space-y-6">
                        ...
                        <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 p-8 rounded-2xl border border-white/10 shadow-2xl">
                            ...
                            {tabs.filter(t => t.id !== 'overview').map((tab) => {
                                ...
                                return (
                                    <button
                                        key={tab.id}
                                        onClick={() => actions.setActiveReport(tab.id as ReportType)}
                                        className="bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 p-5 rounded-2xl hover:border-indigo-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all group relative overflow-hidden"
                                    >
                                        <div className="absolute -right-2 -bottom-2 opacity-5 group-hover:opacity-10 transition-opacity"><Icon size={48} /></div>
                                        <div className="flex flex-col items-center text-center">
                                            <div className="w-10 h-10 bg-slate-950 text-white rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 transition-transform shadow-lg">
                                                <Icon size={18} />
                                            </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
                            {[
                                ...
                                <div key={i} className={cn("relative overflow-hidden aspect-square p-3 flex flex-col justify-between text-white rounded-2xl shadow-md bg-gradient-to-br", stat.color)}>
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm p-6">
                             <div className="flex items-center justify-between mb-6">
                                 ...
                                 <span className="text-[10px] font-black text-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 px-2 py-0.5 rounded-lg uppercase">Live Analysis</span>
                             </div>
                             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                                {state.subjectPieData.slice(0, 6).map((s, i) => {
                                    ...
                                    return (
                                        <div key={i} className="flex flex-col gap-2 p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-700 rounded-xl group hover:bg-white dark:hover:bg-slate-800 transition-colors">
                                            <div className="flex items-center justify-between">
                                                <div className={cn("w-2 h-2 rounded-xl", colors[i])} />
                                            ...
                                            <div className="w-full h-1 bg-slate-100 dark:bg-slate-900 rounded-lg mt-1">
