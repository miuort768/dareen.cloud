    ...
    const renewalsData = students?.flatMap(student =>
        (student.enrollments || []).map(enroll => {
            ...
            if (student.parentPhone) {
                const msg = ` –ﬂÌ— „‰ œ«—Ì‰ «·”«»⁄…: «·„ »ﬁÌ ›Ì —’Ìœ «·ÿ«·» ${student.name} ›Ì „«œ… ${enroll.subject} ÂÊ ${remaining} Õ’’ ›ﬁÿ. Ì—ÃÏ «· ÃœÌœ ·÷„«‰ «” „—«— «·„Ê«⁄Ìœ.`;
            ...
            <div className="mx-auto px-2 space-y-4">
                <div className="bg-gradient-to-br from-[var(--bg-primary)] to-[var(--bg-primary)] shadow-lg px-5 md:px-7 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4 rounded-2xl mt-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-lg font-bold text-on-primary leading-tight"> ﬁ›Ì· «·Õ”«»«  Ê«·√‰‘ÿ… «·„«·Ì…</h1>
                            <p className="text-[10px] font-bold text-white/70 mt-0.5">«· Õ·Ì· «·„«·Ì Ê«·Œ «„Ì ··› —… «·Õ«·Ì…</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 no-print">
                        ...
                        <button onClick={handleRefresh} className="p-1.5 rounded-xl bg-white/15 backdrop-blur-sm hover:bg-white/30 transition-colors" aria-label=" ÕœÌÀ">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <StatItem
                        ...
                        icon={TrendingUp}
                        color="#6C4BFF"
                        subValue={`${totalProjectedIncome > 0 ? ((netProjectedProfit / totalProjectedIncome) * 100).toFixed(0) : 0}% Â«„‘ —»Õ`}
                    />
                    <StatItem
                        title="«· Õ’Ì·«  «·‰ﬁœÌ…"
                        ...
                        icon={Wallet}
                        color="#10B981"
                        subValue={`’«›Ì «·”ÌÊ·…: ${netActualCashFlow.toLocaleString()}`}
                    />
                    <StatItem
                        title="—Ê« » «·„⁄·„« "
                ...
                <div className="px-0 mb-4">
                    <div className="bg-white dark:bg-primary-active rounded-2xl border border-border/50 dark:border-border/50 p-1 flex overflow-x-auto no-scrollbar gap-1 shadow-sm">
                        {[
                            { id: 'payroll', label: '«·—Ê« »', icon: Receipt },
                            { id: 'collections', label: '«· Õ’Ì·« ', icon: Wallet },
                            { id: 'renewals', label: '«· ÃœÌœ« ', icon: AlertCircle },
                            { id: 'analysis', label: ' Õ·Ì· «·„Ê«œ', icon: BarChart3 },
                            { id: 'teachers', label: '√œ«¡ «·ÂÌ∆…', icon: Users },
                            { id: 'compensation', label: '«· ⁄ÊÌ÷« ', icon: RefreshCw },
                            { id: 'summary', label: '«·„·Œ’', icon: TrendingUp }
                        ].map(tab => (
                            <button
                                key={tab.id}
