  ...
  <AnimatePresence>
    {showDetails && appointment && (
      <motion.div
        ...
        exit={{ opacity: 0, x: 30 }}
        className="sticky top-4 h-fit overflow-hidden rounded-none border border-border bg-card"
      >
        <div className="flex items-center justify-between rounded-none bg-primary px-4 py-3 text-on-primary">
          <div>
            <p className="text-micro font-bold text-white/60">تفاصيل الموعد</p>
            <h3 className="text-base font-bold">{appointment.day}</h3>
          </div>
          <div className="flex items-center gap-2">
            <div className="rounded-none bg-white/15 px-3 py-1 text-center">
            ...
            <button
              onClick={onClose}
              className="flex h-8 w-8 items-center justify-center rounded-none text-white/60 transition-all hover:bg-white/15 hover:text-on-primary"
              aria-label="إغلاق"
            >
              <X size={14} />
            </button>
        ...
        <div className="space-y-3 p-4">
          ...
          <div className="flex items-center justify-between rounded-none border-e-[3px] border-e-warning bg-warning-soft p-3 dark:border-e-primary dark:bg-primary-soft">
            <div>
              ...
              <span className="mt-1 inline-block rounded-none bg-surface px-1.5 py-0.5 text-micro font-bold text-warning dark:text-primary">
          ...
          <button
            onClick={onClose}
            className="flex w-full items-center justify-center gap-2 rounded-none bg-primary py-3 text-xs font-bold text-on-primary transition-all hover:bg-primary-hover active:scale-95"
          >
