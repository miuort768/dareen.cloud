...
export const RecentActivity = ({ pointLogs }: RecentActivityProps) => {
    ...
    return (
        <div className="bg-card dark:bg-[#0d0d0f] border border-border dark:border-[#D4AF37]/20 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-main dark:text-white mb-3">آخر النشاطات</h3>
                        ...
                        <div key={log.id || `log-${i}`} className="flex items-start gap-3 relative">
                            ...
                            <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
                                <div className="min-w-0">
                                    <p className="text-xs font-bold text-main dark:text-white truncate">{log.action}</p>
                                    {log.date && (
                                        <p className="text-micro text-muted dark:text-zinc-400 flex items-center gap-1">
