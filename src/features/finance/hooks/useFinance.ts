...
export const useFinance = () => {
  ...
  const handleClearAllFixedExpenses = async () => {
    if (
      !(await confirm({
        title: 'تصفير جميع المبالغ',
        description: 'هل أنت متأكد من تصفير جميع المبالغ؟',
        confirmText: 'تصفير',
        cancelText: 'إلغاء',
      }))
    )
  ...
  const handleDeleteAllTransactions = async () => {
    if (
      !(await confirm({
        ...
        description: 'هل أنت متأكد من حذف جميع المعاملات اليدوية؟ لا يمكن التراجع عن هذا الإجراء.',
        confirmText: 'حذف',
        cancelText: 'إلغاء',
      }))
    )
  ...
  const reportCurrency = useMemo(() => {
    ...
    return 'EGP' // match server default (currencyService.js)
  }, [serverStats])
  ...
  const stats = useMemo(() => {
    ...
    const monthExtraManualExpenses = filteredManualTx
