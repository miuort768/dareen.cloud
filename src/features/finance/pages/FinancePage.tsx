...
export const Finance = () => {
    ...
    if (state.loading) {
        return (
            <div className="space-y-4 p-4 bg-[#F8F7FF] dark:bg-slate-950 min-h-full">
    ...
    return (
        <div className="min-h-full pb-24 overflow-x-hidden relative font-sans bg-[#F8F7FF] dark:bg-slate-950" dir="rtl">
            <div className="relative z-10 max-w-[1600px] mx-auto px-2">
                ...
                {/* ── Header ── */}
                <div className="bg-gradient-to-br from-[#6C4BFF] to-[#8B5CF6] shadow-lg px-5 md:px-7 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 rounded-2xl mt-4">
                    ...
                    <div className="flex items-center gap-2 no-print">
                        <button onClick={() => navigate('/monthly-closing')} className="flex items-center gap-2 h-9 px-4 bg-white/15 backdrop-blur-sm hover:bg-white/30 text-white text-[10px] font-bold transition-all rounded-xl shadow-sm border border-white/10">
                            <CalendarCheck size={13} />
                            <span className="hidden sm:inline whitespace-nowrap">تسوية الشهر</span>
                        </button>
                        <button onClick={() => actions.setShowAddModal(true)} className="flex items-center gap-2 h-9 px-4 bg-white/20 backdrop-blur-sm hover:bg-white/30 text-white text-[10px] font-bold transition-all shadow-sm active:scale-95 rounded-xl">
                            <Plus size={13} />
                            تسجيل معاملة
                        </button>
                        <button onClick={() => {
                        ...
                        }} className="w-9 h-9 flex items-center justify-center bg-white/15 backdrop-blur-sm hover:bg-white/30 text-white transition-all rounded-xl shadow-sm border border-white/10">
                            <Download size={14} />
                        </button>
                    </div>
                </div>
                ...
                <div className="py-5 space-y-6">
                    <div className="bg-white dark:bg-slate-900 border border-slate-100/50 dark:border-slate-800/50 p-5 md:p-6 shadow-sm rounded-2xl">
                        <FinanceStats
