...
const StatusBadge = ({ status }: { status: string }) => {
    if (status === 'completed') return <span className="inline-flex items-center gap-1 text-[9px] font-black text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded-none uppercase tracking-wide"><CheckCircle2 size={9} /> معتمدة</span>;
    if (status === 'pending') return <span className="inline-flex items-center gap-1 text-[9px] font-black text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded-none uppercase tracking-wide"><Clock size={9} /> مراجعة</span>;
    return <span className="inline-flex items-center gap-1 text-[9px] font-black text-slate-400 bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded-none uppercase tracking-wide"><X size={9} /> ملغاة</span>;
};
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-none shadow-sm overflow-hidden" dir="rtl">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest">سجل العمليات المالية</h2>
                ...
                <button
                    onClick={onDeleteAll}
                    className="flex items-center gap-2 px-4 py-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 font-black text-[9px] rounded-none border border-rose-100 dark:border-rose-800 hover:bg-rose-600 hover:text-white hover:border-rose-600 transition-all uppercase tracking-widest"
                >
                    <Trash2 size={12} />
                    تصفير الأرشيف
                </button>
                ...
                <table className="w-full text-right">
                    <thead>
                        <tr className="bg-slate-900 text-white">
                            <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest text-center">#</th>
                            <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest text-center">النوع</th>
                            <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest">البيان</th>
                            <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest text-center">التصنيف</th>
                            <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest text-center">التاريخ</th>
                            <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest text-center">القيمة</th>
                            <th className="px-5 py-3 text-[9px] font-black uppercase tracking-widest text-center">الحالة</th>
                        </tr>
                    </thead>
                        ...
                        {pageTx.length > 0 ? pageTx.map((tx, idx) => {
                            ...
                            return (
                                <tr key={tx.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td className="px-5 py-3">
                                        ...
                                        <p className="text-[9px] text-slate-400 font-bold mt-0.5 font-mono">#{tx.id.substring(0, 8)}</p>
                                    </td>
                                    <td className="px-5 py-3 text-center">
                                        <span className={cn(
                                            "inline-block px-2 py-0.5 text-[9px] font-black rounded-none uppercase tracking-wide",
                                            tx.type === 'income'
                                    ...
                                    <td className="px-5 py-3 text-center">
                                        <span className={cn(
                                            ...
                                            <span className="text-[9px] font-bold mr-0.5">{CURRENCY_SYMBOL}</span>
                                        </span>
                                    </td>
                                    <td className="px-5 py-3 text-center">
                            ...
                            <tr>
                                <td colSpan={7} className="px-6 py-16 text-center">
                                    ...
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">لا توجد عمليات مسجلة</p>
                                </td>
                            </tr>
                        )}
                    </tbody>
                    ...
                    return (
                        <div key={tx.id} className="p-4 flex items-center gap-3">
                            <div className={cn(
                                ...
                                <span className="absolute -top-1 -right-1 text-[8px] font-black bg-slate-900 text-white w-4 h-4 flex items-center justify-center rounded-full">{globalIdx}</span>
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-0.5">
                                    <h3 className="font-bold text-xs text-slate-800 dark:text-white truncate">{tx.description || 'معاملة'}</h3>
                                    <span className={cn("text-sm font-black font-mono mr-2 shrink-0", tx.type === 'income' ? 'text-emerald-500' : 'text-rose-500')}>
                                        {tx.type === 'income' ? '+' : '-'}{tx.amount.toLocaleString()}
                                    </span>
                                </div>
