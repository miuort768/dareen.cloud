            ...
            <div className="flex-1 space-y-2 overflow-y-auto custom-scrollbar">
                {tasks.length > 0 ? (
                    tasks.slice(0, 5).map((task) => (
                        <div key={task.id} className={cn(
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-[13px] font-bold text-main dark:text-main truncate">{task.title}</p>
                                <div className="flex items-center gap-2 mt-0.5">
                                    <span className="text-[11px] font-medium text-muted dark:text-muted">{task.dueDate}</span>
                                    {task.priority === 'high' && (
                    ...
                    <div className="flex flex-col items-center justify-center py-10 opacity-50">
                        <div className="w-10 h-10 rounded-xl bg-surface dark:bg-hover flex items-center justify-center mb-2">
                            <ListTodo size={18} className="text-dim dark:text-dim" />
                        </div>
                        <p className="text-[11px] font-bold text-muted dark:text-muted">لا توجد مهام نشطة حالياً</p>
                    </div>
                )}
            </div>
