    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            <div className="lg:col-span-3 bg-white border border-error dark:bg-error/20 dark:border-error/30 overflow-hidden shadow-sm relative group rounded-2xl">
                <div className="absolute top-0 start-0 w-1.5 h-full bg-error group-hover:w-2 transition-all"></div>
                <div className="p-5 border-b border-error dark:border-error/20 flex items-center justify-between bg-gradient-to-l from-[var(--bg-error)]/80 to-transparent dark:from-[var(--bg-error)]/10">
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            <div className="absolute -inset-1 bg-error/20 blur opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            <div className="relative p-2.5 bg-error shadow-lg shadow-error/20 flex items-center justify-center">
                                <Bell size={22} className="text-on-primary animate-pulse" />
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col items-end gap-1">
                        ...
                        <p className="text-micro font-normal text-muted dark:text-muted uppercase tracking-widest pe-1">قائمة التجديد</p>
                    </div>
                </div>
                        ...
                        <tbody>
                            {lowBalanceStudents.length > 0 ? (
                                lowBalanceStudents.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-error-light/20 dark:hover:bg-error/5 transition-colors border-b last:border-0 border-border dark:border-border/50">
                                        <td className="px-6 py-4 text-center">
                                            <span className="font-medium text-main dark:text-on-primary text-xs">{item.studentName}</span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className="text-xs font-normal text-muted dark:text-muted">{item.subject}</span>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                ...
                                <tr>
                                    <td colSpan={4} className="px-6 py-12 text-center text-xs font-medium text-muted opacity-30">لا توجد اشتراكات منتهية حالياً</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="sm:hidden p-4 space-y-4">
                    {lowBalanceStudents.length > 0 ? (
                        lowBalanceStudents.map((item, idx) => (
                            <div key={idx} className="bg-white dark:bg-card/50 p-4 border border-error dark:border-error/20 shadow-sm relative overflow-hidden">
                        ...
                        <div className="py-10 text-center text-xs font-medium text-muted opacity-30">لا توجد اشتراكات منتهية</div>
                    )}
                </div>
            </div>
            ...
            <div className="bg-white border border-border p-6 dark:bg-primary-active dark:border-border shadow-sm relative overflow-hidden flex flex-col justify-between rounded-2xl">
                <div className="absolute top-0 start-0 w-1 h-full bg-success"></div>
                <div>
