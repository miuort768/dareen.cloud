...
export const StudentQuickBrief = ({ isOpen, onClose, onGenerateReport, student, enrollment, recentSessions }: StudentQuickBriefProps) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm" dir="rtl">
            <div className="bg-white dark:bg-slate-900 border-2 border-slate-950 w-full max-w-xl rounded-none shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] flex flex-col max-h-[90vh] overflow-hidden">
                ...
                <div className="p-6 border-b-2 border-slate-950 bg-slate-50 dark:bg-slate-800">
                    <div className="flex items-start justify-between">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{student.name}</h3>
                                <div className="flex items-center gap-2 mt-2">
                                    <span className="px-2 py-0.5 bg-slate-950 text-white text-[9px] font-black uppercase">{student.grade}</span>
                                    <span className="px-2 py-0.5 bg-amber-400 text-amber-950 border-2 border-slate-950 text-[9px] font-black uppercase flex items-center gap-1">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                    ...
                    {enrollment?.nextSessionNotes && (
                        <div className="bg-amber-50 dark:bg-amber-900/10 p-5 border-2 border-amber-500 rounded-none relative">
                             ...
                             <p className="text-[9px] font-black text-amber-600 uppercase mb-2">NEXT SESSION PREP</p>
                             <p className="text-sm font-bold text-amber-950 dark:text-amber-400 leading-tight">"{enrollment.nextSessionNotes}"</p>
                        </div>
                    )}
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <h4 className="text-[10px] font-black uppercase">Guardian Context</h4>
                        </div>
                        <div className="text-sm font-bold text-slate-900 dark:text-gray-300 leading-relaxed bg-slate-50 dark:bg-slate-800/40 p-5 border-2 border-slate-950 rounded-none">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <h4 className="text-[10px] font-black uppercase">Recent Learning Path</h4>
                        </div>
                            ...
                            {recentSessions.length > 0 ? recentSessions.map((sess, idx) => (
                                <div key={idx} className="p-4 bg-white dark:bg-slate-800 border-2 border-slate-950/10 hover:border-slate-950 transition-all rounded-none flex items-center justify-between group">
                                    <div className="min-w-0">
                                        <p className="text-[9px] font-black text-slate-400 uppercase mb-1">{sess.date}</p>
                                        <p className="text-sm font-black text-slate-900 dark:text-white truncate uppercase tracking-tight">{sess.topics}</p>
                                ...
                                <div className="py-12 text-center border-2 border-dashed border-slate-300 rounded-none bg-slate-50">
                                    <p className="text-[10px] text-slate-400 font-black uppercase">Initialization phase</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="p-6 bg-slate-50 dark:bg-slate-950 border-t-2 border-slate-950 flex gap-4">
                    <button 
                        ...
                        className="flex-1 h-12 bg-emerald-600 text-white rounded-none font-black text-[11px] uppercase border-2 border-slate-950 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all active:translate-y-0 active:shadow-none flex items-center justify-center gap-2"
                    >
                        <Sparkles size={16} />
                        إصدار تقرير شهري
                    </button>
                    <button 
                        onClick={onClose}
                        className="px-8 h-12 bg-slate-950 text-white rounded-none font-black text-[11px] uppercase border-2 border-slate-950 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all active:translate-y-0 active:shadow-none"
                    >
                        إغلاق
                    </button>
                </div>
