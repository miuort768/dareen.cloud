        ...
        <div className="bg-white dark:bg-slate-900 border-2 border-slate-950 dark:border-slate-800 p-6 rounded-none shadow-sm flex flex-col h-full" dir="rtl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-sm font-black text-slate-950 dark:text-white uppercase tracking-tighter">تجديد الاشتراكات</h3>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Financial Monitoring</p>
                    </div>
                </div>
                <div className="flex items-center gap-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 border border-rose-100 dark:border-rose-900/20 px-3 py-1.5 rounded-none text-[10px] font-black uppercase tracking-widest">
                    <AlertCircle size={12} />
            ...
            <div className="space-y-3 overflow-y-auto max-h-[400px] pr-2 -mr-2 custom-scrollbar">
                {lowBalanceStudents.length > 0 ? (
                        ...
                        <div key={idx} className="p-4 bg-slate-50 dark:bg-slate-800/30 rounded-none border-b border-slate-100 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all flex items-center justify-between group">
                            <div className="flex items-center gap-4 min-w-0">
                                <div className="relative">
                                    <div className="w-10 h-10 rounded-none bg-white dark:bg-slate-800 flex items-center justify-center font-black text-xs text-indigo-600 border-2 border-slate-950">
                                ...
                                <div className="min-w-0">
                                    <h4 className="font-black text-xs text-slate-950 dark:text-white truncate group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{item.studentName}</h4>
                                    <p className="text-[10px] font-black text-slate-400 truncate mt-1 uppercase tracking-widest">{item.subject}</p>
                                </div>
                            </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-24 opacity-20 flex flex-col items-center">
                         ...
                         <p className="text-[10px] font-black uppercase tracking-[0.2em]">لا توجد أرصدة منخفضة</p>
                    </div>
                )}
            </div>

            <div className="mt-auto pt-6">
                <button className="w-full h-11 flex items-center justify-center gap-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 hover:text-white rounded-none transition-all active:scale-95 border border-slate-200 dark:border-slate-700">
                    عرض كافة السجلات
                    <ChevronLeft size={16} />
                </button>
            </div>
