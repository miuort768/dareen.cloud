...
export const ActivityTimeline = ({ sessions, tasks }: ActivityTimelineProps) => {
    ...
    const variantStyles: Record<string, { dot: string; badge: string; iconBg: string; iconText: string }> = {
        success: { dot: 'bg-success', badge: 'bg-success-soft text-success border-success/20', iconBg: 'bg-success-soft', iconText: 'text-success' },
        error: { dot: 'bg-error', badge: 'bg-error-soft text-error border-error/20', iconBg: 'bg-error-soft', iconText: 'text-error' },
        info: { dot: 'bg-info', badge: 'bg-info-soft text-info border-info/20', iconBg: 'bg-info-soft', iconText: 'text-info' },
        warning: { dot: 'bg-warning', badge: 'bg-warning-soft text-warning border-warning/20', iconBg: 'bg-warning-soft', iconText: 'text-warning' },
    };
    ...
    return (
        <div className="rounded-2xl bg-card border border-border p-5 font-dash" dir="rtl">
            <div className="flex items-center justify-between mb-4">
                ...
                {allItems.length > 0 && (
                    <Badge variant="default" className="text-[10px] h-5 px-2.5 rounded-lg bg-info-soft text-info border-info/20">
                ...
                <div className="space-y-2">
                    {allItems.map((item) => {
                        ...
                        return (
                            <div
                                key={item.id}
                                className="flex items-center gap-3 p-3 rounded-xl bg-surface hover:bg-hover transition-colors"
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="text-[11px] font-bold text-main truncate">{item.title}</h4>
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        <Clock size={9} className="text-muted shrink-0" />
