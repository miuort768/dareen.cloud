            ...
            <div className="bg-card dark:bg-card border-2 border-border dark:border-primary/20 w-full max-w-2xl rounded-none shadow-[16px_16px_0px_0px_black] flex flex-col max-h-[95vh] overflow-hidden relative">
                ...
                <div className="flex-1 overflow-y-auto p-10 pb-6 custom-scrollbar relative z-10">
                    ...
                    <div className="flex flex-col items-center justify-center text-center space-y-6 mb-12">
                        ...
                        <div>
                            <h3 className="text-xl font-medium text-main dark:text-main uppercase tracking-tight italic"> ﬁ—Ì— «· „Ì“ «·√ﬂ«œÌ„Ì</h3>
                            <p className="text-micro font-medium text-muted dark:text-muted uppercase mt-1">„‰’… {academyName} «· ⁄·Ì„Ì… ó {new Date().toLocaleDateString('ar-EG', { month: 'long', year: 'numeric' })}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="flex items-center justify-between p-6 bg-background dark:bg-surface border-2 border-border dark:border-primary/20 rounded-none">
                            <div className="space-y-1 text-start">
                                <p className="text-micro font-medium text-primary dark:text-primary uppercase">‰Ã„… √ﬂ«œÌ„Ì…</p>
                            ...
                            <div className="text-end bg-card dark:bg-card px-4 py-2 border-2 border-border dark:border-primary/20 rounded-none">
                                <p className="text-micro font-medium text-muted dark:text-muted uppercase mb-0.5">«·„” ÊÏ / «·„«œ…</p>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            {[
                                { label: '«·Õ÷Ê—', value: `${student.attendance}%`, icon: ShieldCheck, color: 'bg-success', onColor: 'text-on-success' },
                                { label: '≈Ã„«·Ì «·‰ﬁ«ÿ', value: student.points, icon: Star, color: 'bg-warning', onColor: 'text-on-warning' },
                                { label: '«·Ã·”« ', value: student.sessionsCompleted, icon: Calendar, color: 'bg-primary', onColor: 'text-on-primary' }
                            ].map((item, idx) => (
                        ...
                        <div className="p-6 bg-background dark:bg-surface text-main dark:text-main rounded-none border-2 border-border dark:border-primary/20 text-center shadow-[6px_6px_0px_0px_var(--bg-primary)]">
                            <p className="text-micro font-medium uppercase italic">‰Õ‰ ›ŒÊ—Ê‰ » ﬁœ„ﬂ «·„” „— Ì« »ÿ·! «” „— ›Ì «· √·ﬁ.</p>
                        </div>
                    </div>
                </div>
