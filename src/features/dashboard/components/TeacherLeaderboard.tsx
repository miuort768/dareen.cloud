                ...
                {students.map((student, index) => (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 flex items-center justify-center font-black text-xs border-2 border-gray-950 ${
                                index === 0 ? 'bg-yellow-400' : index === 1 ? 'bg-gray-300' : index === 2 ? 'bg-orange-400' : 'bg-white text-gray-400'
                            }`}>
                                {index + 1}
                            </div>
                            <div>
                                <h4 className="text-xs font-black text-gray-900 dark:text-white">{student.name}</h4>
                                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{student.grade}</p>
                            </div>
                        </div>
                        
                        <div className="flex items-center gap-2">
                            <div className="flex -space-x-1 rtl:space-x-reverse overflow-hidden">
                                {student.badges && typeof student.badges === 'string' && JSON.parse(student.badges).slice(0, 3).map((badge: any, bIdx: number) => (
                                    <div key={bIdx} title={badge.name} className="w-5 h-5 rounded-full bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 flex items-center justify-center shadow-sm">
                                        <Award size={10} className="text-primary-600" />
                                    </div>
                                ))}
                            </div>
