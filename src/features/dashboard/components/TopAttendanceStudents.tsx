...
export const TopAttendanceStudents = ({ sessions, onStudentClick }: TopAttendanceStudentsProps) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-3.5 shadow-sm border border-slate-100 dark:border-slate-700">
            <div className="flex items-center justify-between mb-2.5">
                <h3 className="text-[11px] font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                ...
                {topPresentStudents.length > 0 ? (
                    topPresentStudents.map((stu, i) => (
                        <div
                            ...
                            className="flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600 hover:border-amber-300 dark:hover:border-amber-500/30 transition-all cursor-pointer"
                        >
                            <div className="flex items-center gap-2">
                                ...
                                <div className="min-w-0">
                                    <p className="text-[10px] font-bold text-slate-800 dark:text-slate-100 truncate">{stu.name}</p>
                                </div>
                            </div>
                            <div className="flex items-baseline gap-1">
                                <span className="text-base font-black text-slate-900 dark:text-white tabular-nums">{stu.count}</span>
                                <span className="text-[7px] font-bold text-amber-500">حصة</span>
                            </div>
                        </div>
                    ))
                ) : (
                    <div className="flex flex-col items-center justify-center py-6 opacity-50">
                        <div className="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center mb-1.5">
                            <User size={14} className="text-slate-300 dark:text-slate-500" />
                        </div>
                        <p className="text-[9px] font-bold text-slate-400">لا توجد سجلات حالياً</p>
                    </div>
                )}
            </div>

            <div className="mt-2.5 bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl p-2.5 text-white flex items-center justify-between">
                <div>
                    <p className="text-[7px] font-bold text-amber-100">إجمالي حصص الشهر</p>
