...
export const TopAttendanceStudents = ({ sessions, onStudentClick }: TopAttendanceStudentsProps) => {
    const topPresentStudents = useMemo(() => {
        ...
        sessions.forEach(s => {
            const isCompleted = ['completed', '„ﬂ „·…', ' „ '].includes(s.status?.toLowerCase());
    ...
    const totalMonthSessions = useMemo(() => {
        ...
        return sessions.filter(s =>
            ['completed', '„ﬂ „·…', ' „ '].includes(s.status?.toLowerCase()) &&
    ...
    return (
        <div>
            ...
            <div className="space-y-1.5">
                {topPresentStudents.length > 0 ? (
                        ...
                        >
                            <div className="flex items-center gap-2">
                                <div className={cn(
                                    ...
                                    i === 1 ? "bg-surface dark:bg-hover text-main dark:text-main" :
                                    i === 2 ? "bg-warning-soft dark:bg-primary/10 text-warning dark:text-primary" :
                                    "bg-surface dark:bg-hover text-muted dark:text-muted"
                                )}>
                                    {i + 1}
                                </div>
                            ...
                            <div className="flex items-baseline gap-1">
                                ...
                                <span className="text-micro font-bold text-warning dark:text-primary">Õ’…</span>
                            </div>
                        </div>
                    ))
                ) : (
                    <div className="flex flex-col items-center justify-center py-6 opacity-50">
                        ...
                        <p className="text-micro font-bold text-muted dark:text-muted">·«  ÊÃœ ”Ã·«  Õ«·Ì«</p>
                    </div>
                )}
            </div>
            ...
            <div className="mt-2.5 bg-warning dark:bg-primary p-2.5 text-on-warning rounded-xl flex items-center justify-between">
                <div>
                    <p className="text-micro font-bold text-on-warning">≈Ã„«·Ì Õ’’ «·‘Â—</p>
