import { Link } from 'react-router-dom';
import {
    ListTodo,
    CreditCard,
    ChevronLeft,
    Clock,
    Phone,
    UserX,
    Briefcase
} from 'lucide-react';
import { cn } from '@/lib/utils';
import type { DashboardTask as Task, LowBalanceStudent, DashboardStats as Stats } from '../types';
import { sendWhatsAppReminder } from '../../../shared/utils/reminders';
import { useAdminPhone } from '../../../context/AppContext';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';

interface OperationsDashboardProps {
    tasks: Task[];
    lowBalanceStudents: LowBalanceStudent[];
    stats: Stats;
}

export const OperationsDashboard = ({ tasks, lowBalanceStudents, stats }: OperationsDashboardProps) => {
    const adminPhone = useAdminPhone();

    return (
        <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-4" dir="rtl">
            {/* 1. Subscriptions & Renewals */}
            <Card>
                <CardContent className="p-5 flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-success/10 text-success">
                                <CreditCard size={20} />
                            </div>
                            <div>
                                <h3 className="text-sm font-bold text-main leading-tight">تجديد الاشتراكات</h3>
                                <p className="text-micro font-medium text-muted">إدارة التحصيل المالي</p>
                            </div>
                        </div>
                        <Badge variant="success">{stats.lowBalanceCount} تنبيهات</Badge>
                    </div>

                    <div className="flex-1 space-y-2 overflow-y-auto max-h-[380px] custom-scrollbar ps-1">
                        {lowBalanceStudents.length > 0 ? (
                            lowBalanceStudents.map((item, idx) => (
                                <div key={idx} className="p-3 bg-card rounded-xl border border-border/50 transition-all flex items-center justify-between group/item">
                                    <div className="flex items-center gap-3 min-w-0">
                                        <div className="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-xs transition-colors bg-success/10 text-success">
                                            {item.studentName.charAt(0)}
                                        </div>
                                        <div className="min-w-0">
                                            <h4 className="font-bold text-xs text-main truncate">{item.studentName}</h4>
                                            <p className="text-micro font-medium text-muted mt-0.5 flex items-center gap-1">
                                                <span className="w-1.5 h-1.5 rounded-full bg-success" />
                                                {item.subject}
                                            </p>
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-3 shrink-0">
                                        <span className={cn(
                                            "text-micro font-bold px-2 py-1 border rounded-xl",
                                            item.remainingSessions === 0
                                                ? "text-error border-error bg-error-soft"
                                                : "text-warning border-warning bg-warning-soft"
                                        )}>
                                            {item.remainingSessions === 0 ? 'منتهي' : `${item.remainingSessions} جلسة`}
                                        </span>
                                        <Button
                                            variant="success"
                                            size="icon"
                                            onClick={() => sendWhatsAppReminder(item, undefined, adminPhone)}
                                            title="إرسال تذكير واتساب"
                                        >
                                            <Phone size={14} />
                                        </Button>
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="text-center py-12 bg-card rounded-xl border border-dashed border-border">
                                <UserX size={24} className="mx-auto mb-2 text-success/40" />
                                <p className="text-micro font-medium text-muted">لا توجد تجديدات معلقة</p>
                            </div>
                        )}
                    </div>

                    <div className="mt-6">
                        <Link to="/students" className="w-full">
                            <Button className="w-full" size="lg">
                                إدارة كافة الطلاب
                                <ChevronLeft size={14} />
                            </Button>
                        </Link>
                    </div>
                </CardContent>
            </Card>

            {/* 2. Tasks & Requests */}
            <Card>
                <CardContent className="p-5 flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-info/10 text-info">
                                <Briefcase size={20} />
                            </div>
                            <div>
                                <h3 className="text-sm font-bold text-main leading-tight">المهام والطلبات</h3>
                                <p className="text-micro font-medium text-muted">سير العمليات التشغيلية</p>
                            </div>
                        </div>
                        <Badge variant="secondary">{tasks.length} مهام نشطة</Badge>
                    </div>

                    <div className="flex-1 space-y-2 overflow-y-auto max-h-[380px] custom-scrollbar ps-1">
                        {tasks.length > 0 ? (
                            tasks.slice(0, 10).map((task) => (
                                <div key={task.id} className="p-3 bg-card rounded-xl border border-border/50 transition-all flex items-center justify-between">
                                    <div className="flex items-center gap-3 min-w-0">
                                        <div className={cn("w-1.5 h-8 rounded-full", task.priority === 'high' ? "bg-error" : task.priority === 'medium' ? "bg-info" : "bg-primary")} />
                                        <div className="min-w-0">
                                            <h4 className="font-bold text-xs text-main truncate">{task.title}</h4>
                                            <div className="flex items-center gap-2 mt-1">
                                                <div className="flex items-center gap-1">
                                                    <Clock size={10} className="text-muted" />
                                                    <span className="text-micro font-medium text-muted">{task.dueDate}</span>
                                                </div>
                                                <span className="text-micro font-medium text-dim">[{task.priority}]</span>
                                            </div>
                                        </div>
                                    </div>

                                    <Link to="/tasks">
                                        <Button variant="ghost" size="icon">
                                            <ChevronLeft size={16} />
                                        </Button>
                                    </Link>
                                </div>
                            ))
                        ) : (
                            <div className="text-center py-12 bg-card rounded-xl border border-dashed border-border">
                                <ListTodo size={24} className="mx-auto mb-2 text-info/40" />
                                <p className="text-micro font-medium text-muted">تم إنجاز كافة المهام</p>
                            </div>
                        )}
                    </div>

                    <div className="mt-6">
                        <Link to="/tasks" className="w-full">
                            <Button variant="success" className="w-full" size="lg">
                                مركز المهام المتكامل
                                <ChevronLeft size={14} />
                            </Button>
                        </Link>
                    </div>
                </CardContent>
            </Card>
        </div>
    );
};

