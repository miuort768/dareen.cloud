  ...
  return (
    <motion.div layout className="relative rounded-2xl border border-border bg-card p-3.5">
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-1.5">
              {student.grade && (
                <span className="rounded-md bg-surface px-1.5 py-0.5 text-micro font-bold text-muted">
                  {student.grade}
                </span>
              )}
        ...
        {todaySlot ? (
          <span className="shrink-0 rounded-lg bg-success-soft px-2 py-1 text-micro font-bold tabular-nums text-success">
            {todaySlot.hour}:00 {periodLabel(todaySlot.period)}
          </span>
        ) : (
          <span className="shrink-0 rounded-lg bg-surface px-2 py-1 text-micro font-bold text-muted">
            بدون موعد اليوم
          </span>
        )}
      </div>
      ...
      <div className="mt-3 flex gap-1.5">
        <motion.button
          ...
          className="flex flex-1 items-center justify-center gap-1 rounded-xl bg-success py-2.5 text-micro font-bold text-on-success focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"
        >
        ...
        <motion.button
          ...
          className="flex items-center justify-center gap-1 rounded-xl bg-primary-soft px-3 py-2.5 text-micro font-bold text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"
        >
        ...
        <button
          ...
          className="flex w-9 items-center justify-center rounded-xl border border-border text-muted transition-colors hover:text-main focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"
        >
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="absolute end-3 top-12 z-20 w-40 overflow-hidden rounded-xl border border-border bg-card shadow-elevation-3"
            role="menu"
          >
            <button
              onClick={() => {
