  ...
  return (
    <motion.div {...fadeUpStatic} className="px-4 pb-1 pt-3">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary via-primary-deep to-primary p-4">
          ...
          <div className="min-w-0 flex-1 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {stats.map((s) => (
                <div
                  key={s.label}
                  className="rounded-xl bg-white/10 px-2 py-1.5 text-center backdrop-blur-sm"
                >
                  <p
                    className={cn(
        ...
        {/* التاريخ */}
        <div className="relative z-10 mt-3 flex items-center justify-between rounded-xl bg-white/10 px-3 py-2 backdrop-blur-sm">
          ...
          {onDateChange && date && (
            <input
              ...
              className="w-[105px] cursor-pointer rounded-lg border-none bg-transparent p-0 text-end text-micro font-bold text-white outline-none"
            />
          )}
        </div>
      </div>
