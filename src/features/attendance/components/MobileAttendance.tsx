...
export const MobileAttendance = () => {
    ...
    return (
        <div
            ...
            dir="rtl"
            className="min-h-full pb-4 overflow-x-hidden relative bg-background"
        >
            {/* Pull to Refresh */}
            <motion.div
            ...
            <motion.div {...fadeUp} className="px-4 pt-3 pb-2">
                <div className="grid grid-cols-3 gap-2">
                    <div className="bg-card rounded-card p-3 text-center shadow-soft border border-success/30">
                        <p className="text-lg font-black text-success tabular-nums leading-none">{completedToday}</p>
                        <p className="text-micro font-bold text-success/70 mt-1">حضور</p>
                    </div>
                    <div className="bg-card rounded-card p-3 text-center shadow-soft border border-error/30">
                        <p className="text-lg font-black text-error tabular-nums leading-none">{cancelledToday}</p>
                        <p className="text-micro font-bold text-error/70 mt-1">غياب</p>
                    </div>
                    <div className="bg-card rounded-card p-3 text-center shadow-soft border border-border/30">
                        <p className="text-lg font-black text-main tabular-nums leading-none">{scheduledToday}</p>
                                        ...
                                        return (
                                            <div key={teacher} className="bg-card rounded-card shadow-soft border border-border/50 overflow-hidden">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div className="w-7 h-7 rounded-xl flex items-center justify-center text-micro font-black bg-primary-soft text-primary">
                                                        {teacher.charAt(0)}
                                                    </div>
                                                    <span className="text-xs font-bold text-main">{teacher}</span>
                                                </div>
                                                <span className="text-micro font-bold px-2 py-0.5 rounded-lg bg-primary-soft text-primary">
                                                            ...
                                                            <span className="text-micro font-bold text-muted px-1 py-0.5 rounded border border-border">{student.grade}</span>
                                                                    ...
                                                                    {session ? (
                                                                        <span className={cn(
                                                                            ...
                                                                            session.status === 'cancelled' ? 'bg-error-soft text-error' :
                                                                            'bg-warning-soft text-warning'
                                                                        )}>
                                                                            {session.status === 'completed' ? 'تم' : session.status === 'cancelled' ? 'غائب' : 'مجدول'}
                                                                        </span>
                                                                    ) : (
                                                                        <span className="text-micro font-bold px-2 py-0.5 rounded-lg bg-warning-soft text-warning">
                                                                            انتظار
                                                                        </span>
                                                                    )}
                                                                </div>
                                        ...
                                        >
                                            <div className="flex items-center gap-2.5">
                                                <div className={cn(
                                                    ...
                                                    session.status === 'cancelled' ? 'bg-error-soft' :
                                                    'bg-warning-soft'
                                                )}>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="text-end">
                    {todaySlot ? (
                        ...
                        <span className="text-micro font-bold text-muted bg-surface px-2 py-1 rounded-lg">
                            بدون موعد
                        </span>
                    )}
                </div>
