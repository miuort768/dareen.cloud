...
const FinancialCard = ({ title, value, subValue, icon: Icon, color, bg, subColor }: { title: string; value: string; subValue?: string; icon: React.ComponentType<{ size?: number }>; color: string; bg: string; subColor?: string }) => (
    <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 rounded-none shadow-sm">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed">
                            ...
                            مما حقق صافي ربح قدره <span className="font-normal text-[#5c59f2]">{netProfit.toLocaleString()} ج.م</span>.
                        </p>
                    </div>
                </div>
            </div>
