                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <div className="relative group">
                                <DollarSign className="absolute end-3 top-1/2 -translate-y-1/2 text-dim group-focus-within:text-success transition-colors" size={12} />
                                <select
                                    value={formData.currency}
                                    onChange={e => setFormData({ ...formData, currency: e.target.value })}
                                    className="w-full px-4 py-2.5 bg-surface border border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 text-main text-xs font-medium transition-all pe-10 appearance-none"
                                >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="space-y-1.5">
                                ...
                                <div className="relative">
                                    <User className="absolute end-3 top-1/2 -translate-y-1/2 text-dim" size={12} />
                                    <input
                                        value={formData.username}
                                        onChange={e => setFormData({ ...formData, username: e.target.value })}
                                        className="w-full ps-4 pe-10 py-2.5 bg-card border border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 text-xs font-medium font-mono text-main transition-all"
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="relative">
                                    <Key className="absolute end-3 top-1/2 -translate-y-1/2 text-dim" size={12} />
                                    <input
                                        type="text"
                                        value={formData.password}
                                        onChange={e => setFormData({ ...formData, password: e.target.value })}
                                        className="w-full ps-4 pe-10 py-2.5 bg-card border border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 text-xs font-medium font-mono tracking-widest text-main transition-all"
...
const FormInput = ({ label, icon: Icon, placeholder, value, onChange, required, type = "text", dir = "rtl" }: { label: string; icon: React.ComponentType<{ size?: number }>; placeholder?: string; value: string; onChange: (val: string) => void; required?: boolean; type?: string; dir?: string }) => (
    <div className="space-y-1.5">
        ...
        <div className="relative group">
            {Icon && <Icon className="absolute end-3 top-1/2 -translate-y-1/2 text-dim group-focus-within:text-primary transition-colors" size={12} />}
            <input
                required={required}
                type={type}
                dir={dir}
