    ...
    return (
        <div className="bg-white/80 dark:bg-primary-active/80 backdrop-blur-xl rounded-2xl shadow-sm border border-border p-4 flex flex-col md:flex-row items-center justify-between gap-4 mb-8" dir="rtl">
            ...
            <div className="relative flex-1 w-full group">
                <Search className="absolute end-3 top-1/2 -translate-y-1/2 text-dim group-focus-within:text-primary transition-colors" size={14} />
                <input
                    type="text"
                    placeholder="ابحث عن معلمة باسمها أو تخصصها..."
                    value={searchTerm}
                    onChange={(e) => onSearchChange(e.target.value)}
                    className="w-full ps-6 pe-10 py-2.5 bg-surface border border-border rounded-xl outline-none text-xs font-bold text-main placeholder:text-dim focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all"
                />
            </div>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                ...
                <div className="flex items-center gap-2 border-e border-border pe-3 ms-1">
