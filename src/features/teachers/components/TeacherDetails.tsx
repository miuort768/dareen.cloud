...
export const TeacherDetails = ({
    ...
    return (
        <div className={cn(
            "bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-xl overflow-hidden flex flex-col",
            // Mobile: Full screen overlay
            "fixed inset-0 z-[100] lg:h-fit lg:sticky lg:top-6 rounded-none lg:shadow-sm lg:border lg:border-gray-200 lg:dark:border-gray-800"
        )}>
                    ...
                    {!isTeacherView && (
                        <>
                            ...
                            <button
                                ...
                                className="p-2 text-primary-600 hover:bg-primary-50 rounded-none transition-colors"
                                title="إرسال تنبيه"
                            >
                                <Bell size={18} />
                            </button>
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                    ...
                    <div className="space-y-2 overflow-y-auto custom-scrollbar flex-1">
                        {enrolledStudents.map(student => {
                            ...
                            return (
                                <div key={student.id} className="group bg-white border border-gray-100 p-3 rounded-none dark:bg-gray-900 dark:border-gray-800 hover:border-primary-200">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex gap-1 group-hover:opacity-100 lg:opacity-0">
                                            <button
                                                onClick={() => onLogAttendance(student, enrollment)}
                                                className="p-1 text-emerald-600 hover:bg-emerald-50 rounded-none"
                                            >
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {teacherSessions.map(session => (
                                <div key={session.id} className="flex items-center justify-between text-xs p-2 bg-gray-50 dark:bg-gray-800/30 rounded-none border border-gray-100 dark:border-gray-700">
                                    <div>
                                        <p className="font-bold">{session.studentName}</p>
                                        <p className="text-[10px] text-gray-500">{session.date}</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className={`px-2 py-0.5 rounded-none text-[10px] ${session.status === 'completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'
                                            }`}>
