...
export const EnrollmentForm = ({ teachers, onSubmit }: EnrollmentFormProps) => {
    ...
    return (
        <div className="pt-8 border-t border-border">
            <h4 className="text-[10px] font-medium text-main uppercase tracking-widest mb-4">إضافة اشتراك جديد</h4>
            <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div className="bg-primary-soft p-3 space-y-3">
                    <p className="text-[9px] font-medium text-primary uppercase">المواعيد</p>
                    <div className="flex gap-2">
                        <select
                            value={slotInput.day}
                            onChange={e => setSlotInput({ ...slotInput, day: e.target.value })}
                            className="flex-1 px-2 py-1 text-[10px] font-normal border border-border bg-card dark:bg-hover dark:text-main"
                        >
                        ...
                        <input
                            ...
                            className="w-20 px-2 py-1 text-[10px] font-normal border border-border bg-card dark:bg-hover dark:text-main"
                        />
                        <button type="button" onClick={handleAddSlot} className="bg-primary text-on-primary px-2"><Plus size={14} /></button>
                    </div>
                    <div className="flex flex-wrap gap-1">
                        {schedule.map((s, idx) => (
                            <div key={idx} className="bg-card px-2 py-1 text-[9px] font-medium border border-primary-soft flex items-center gap-1 dark:bg-hover shadow-sm">
                                {s.day} {s.hour}
