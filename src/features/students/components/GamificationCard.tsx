        ...
        <div className="space-y-3 md:space-y-4">
            ...
            <div className="bg-primary p-3 md:p-4 shadow-sm relative overflow-hidden group rounded-2xl">
                ...
                <div className="flex items-center justify-between relative z-10">
                    <div>
                        <p className="text-[7px] md:text-[8px] font-medium text-on-primary uppercase tracking-widest mb-0.5 opacity-80">إجمالي نقاط الإنجاز</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                ...
                <div className="bg-card border border-border p-3 md:p-4 shadow-sm rounded-2xl">
                    <h3 className="font-medium text-[8px] md:text-[9px] uppercase tracking-widest text-dim mb-3 flex items-center gap-1.5">
                        ...
                        {pointLogs.length > 0 ? pointLogs.map((log) => (
                            <div key={log.id} className="flex items-center justify-between p-2 bg-surface dark:bg-hover border border-border transition-colors hover:border-primary dark:hover:border-primary rounded-xl">
                                <div className="min-w-0">
                                    <p className="text-[8px] md:text-[9px] font-medium text-main truncate">{log.action}</p>
                                    <p className="text-[6px] md:text-[7px] text-dim font-normal mt-0.5">{format(new Date(log.timestamp), 'yyyy/MM/dd', { locale: ar })}</p>
                                </div>
                                <span className={cn(
                            ...
                            <p className="text-center py-6 text-[7px] font-normal text-dim uppercase tracking-widest">لا توجد سجلات</p>
                        )}
                    </div>
                </div>
            </div>
