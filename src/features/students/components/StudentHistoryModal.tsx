...
export const StudentHistoryModal = ({ student, onClose }: StudentHistoryModalProps) => {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-900 w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl animate-in fade-in zoom-in duration-300">
                <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                    <div>
                        <h3 className="font-black text-xl text-gray-900 dark:text-white">سجل الدوام الكامل</h3>
                        <p className="text-xs font-bold text-gray-500 mt-1">للطالب: {student.name}</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900/50 custom-scrollbar">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-64 text-gray-400">
                            ...
                            <p className="font-bold">لا يوجد سجل حصص لهذا الطالب</p>
                        </div>
                    ) : (
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                    {sessions.map(session => (
                                        <tr key={session.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                            <td className="p-4 font-mono font-bold">{new Date(session.date).toLocaleDateString('ar-EG')}</td>
                                            <td className="p-4 font-bold">{session.day}</td>
                                            <td className="p-4 font-bold text-gray-900 dark:text-white">{session.subject}</td>
