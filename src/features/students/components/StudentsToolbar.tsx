...
export const StudentsToolbar = ({ filteredCount, totalCount, onDeleteAll }: StudentsToolbarProps) => {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!/\.(csv|txt)$/i.test(file.name)) {
      showNotification('يرجى رفع ملف CSV أو TXT فقط', 'error');
      e.target.value = '';
      return;
    }
    setImporting(true);
    try {
      ...
      const hasHeader = headers.some(h => ['name', 'الاسم', 'اسم الطالب', 'grade', 'المرحلة', 'الصف', 'parentphone', 'parent_phone', 'هاتف', 'جوال'].includes(h));
      ...
      for (const row of dataRows) {
        ...
        if (hasHeader) {
          ...
          const pi = idx(['parentphone', 'parent_phone', 'هاتف ولي الامر', 'هاتف ولي الأمر', 'جوال ولي الامر', 'هاتف', 'جوال']);
          name = ni >= 0 ? (row[ni] || '') : '';
          grade = gi >= 0 ? (row[gi] || '') : '';
        ...
        try {
          await api.post('/students', { name: name.trim(), grade: grade.trim(), parentPhone: parentPhone.trim() });
          added++;
        } catch {
          skipped++;
        }
      ...
      if (added === 0) {
        ...
        showNotification(`تم استيراد ${added} طالب بنجاح${skipped ? `، وتجاوز ${skipped}` : ''}`, 'success');
      }
    } catch {
    ...
    <div className="bg-card border border-border rounded-2xl p-3 shadow-elevation-1">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <input type="file" ref={fileInputRef} accept=".csv,.txt" className="hidden" onChange={handleImport} />
          <button onClick={() => fileInputRef.current?.click()} disabled={importing} className={cn(btnClass, 'bg-surface border border-border text-muted hover:bg-hover', importing && 'opacity-60')}>
            {importing ? <Loader2 size={12} className="animate-spin" /> : <Upload size={12} />} <span className="hidden sm:inline">استيراد</span>
          </button>
