...
const StatItem = ({ label, value, icon: Icon, color, symbol, bg, borderAccent }: { label: string, value: string | number, icon: any, color: string, symbol: string, bg: string, borderAccent: string }) => (
    <div className={cn("bg-white dark:bg-slate-900 border-x border-b border-slate-100 dark:border-slate-800 p-3 rounded-xl shadow-sm flex flex-col items-center text-center hover:shadow-md transition-shadow relative overflow-hidden", borderAccent)}>
        ...
        <div className={cn("w-6 h-6 rounded-lg flex items-center justify-center mb-1.5 relative z-10", bg)}>
