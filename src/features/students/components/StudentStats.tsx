...
import { Users, UserCheck, GraduationCap, BookOpen, TrendingUp, TrendingDown } from 'lucide-react';
...
const primaryStat = {
  label: 'الطلاب',
  icon: Users,
  value: (p: StudentStatsProps) => p.totalStudents,
  prev: (p: StudentStatsProps) => p.previousStudents ?? Math.max(0, p.totalStudents - 2),
...
const secondaryStats = [
  {
    label: 'الاشتراكات', icon: UserCheck,
    value: (p: StudentStatsProps) => p.activeEnrollments,
    prev: (p: StudentStatsProps) => p.previousEnrollments ?? Math.max(0, p.activeEnrollments - 5),
    bg: 'bg-success-soft', text: 'text-success', iconBg: 'bg-success/10', ring: 'ring-success/20',
  },
  {
    label: 'المراحل', icon: GraduationCap,
    value: (p: StudentStatsProps) => p.uniqueGrades,
    prev: () => 0,
    bg: 'bg-primary-soft', text: 'text-primary', iconBg: 'bg-primary/10', ring: 'ring-primary/20',
  },
  {
    label: 'متوسط الحصص', icon: BookOpen,
    value: (p: StudentStatsProps) => p.averageSessionsPerStudent,
    prev: () => 0,
    bg: 'bg-warning-soft', text: 'text-warning', iconBg: 'bg-warning/10', ring: 'ring-warning/20',
  },
];

export const StudentStats = (props: StudentStatsProps) => {
      ...
      >
        <div className="absolute inset-0 opacity-[0.06]">
          <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="st-stats-grid" x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse">
                <circle cx="2" cy="2" r="1" fill="white" />
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#st-stats-grid)" />
          </svg>
        </div>
        <div className="relative z-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Sparkline data={[pPrev, pVal]} color="rgba(255,255,255,0.5)" />
        ...
        {secondaryStats.map((stat, i) => {
          ...
          return (
            <motion.div
              ...
              className="p-3 rounded-2xl border border-border bg-card shadow-elevation-1 hover:shadow-elevation-2 transition-all duration-300"
            >
