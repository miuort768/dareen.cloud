...
interface PoliciesSettingsProps {
    backdateLockEnabled: boolean; setBackdateLockEnabled: (v: boolean) => Promise<void> | void;
    showNotify: (msg: string) => void; teacherCommissionType: string;
    setTeacherCommissionType: (v: string) => Promise<void> | void;
    autoFreezeThreshold: number; setAutoFreezeThreshold: (v: number) => Promise<void> | void;
...
export const PoliciesSettings = ({
    ...
    autoFreezeThreshold, setAutoFreezeThreshold, setSecureAction
                ...
                <div className="p-4 bg-background border border-border/20 rounded-xl">
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                            ...
                            <p className="text-[11px] text-error/70 mt-1.5 leading-relaxed">
                                يمنع الموظفين من إضافة أو تعديل حصص في تواريخ قديمة لضمان دقة السجلات المالية.
                            </p>
                        </div>
                        <Toggle checked={backdateLockEnabled} onChange={() => setBackdateLockEnabled(!backdateLockEnabled).then(() => showNotify('تم تحديث خيار الحماية'))} />
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {[
                            ...
                            <button
                                ...
                                className={cn(
                                    'p-3.5 border rounded-xl text-start transition-all',
                            ...
                            >
                                ...
                                <p className={cn('text-[11px] mt-0.5', teacherCommissionType === opt.id ? 'text-on-primary/60' : 'text-muted')}>{opt.sub}</p>
                                {teacherCommissionType === opt.id && <CheckCircle2 size={12} className="mt-1.5 text-on-primary/70" />}
                            </button>
                        ))}
                    </div>
                </div>
        ...
        <SectionCard>
            ...
            <div className="space-y-4">
                <div className="p-4 bg-background border border-border/20 rounded-xl">
                    ...
                    <p className="text-[11px] text-info/70 leading-relaxed mb-3">
                        إذا تجاوز الطالب هذا العدد من مرات الغياب المتعاقبة، يتم تجميد اشتراكه تلقائياً.
                    </p>
                    <div className="flex items-center gap-3">
                        <InputField type="number" value={autoFreezeThreshold} onChange={e => setAutoFreezeThreshold(Number(e.target.value))} min={1} max={15} className="w-20 text-center font-bold text-lg" />
                        <PrimaryBtn onClick={() => setAutoFreezeThreshold(autoFreezeThreshold).then(() => showNotify('تم حفظ السياسة'))} className="flex-1">
                ...
                <div className="p-4 bg-background border border-error/10 rounded-xl">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] text-error/60">منطقة الخطر</p>
                        </div>
                    </div>
