...
export const settingsService = {
  ...
  async updateCurrency(
    code: string,
    data: { name: string; symbol: string; isActive: number; sortOrder: number },
  ) {
  ...
  async getAuditLogs() {
    return api.get<AuditLogEntry[]>('/system/audit-logs')
  },
  ...
  async getBackupHistory(page = 1, limit = 20) {
    return api.get<{
      ...
      limit: number
      totalPages: number
    }>(`/system/backup-history?page=${page}&limit=${limit}`)
  },
