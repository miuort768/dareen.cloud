...
export const HistoryModal = ({ student, evaluations, canDelete, onDelete, onClose }: HistoryModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-[10001] md:animate-in md:fade-in">
            <div className="bg-white dark:bg-slate-900 shadow-2xl w-full max-w-xl flex flex-col max-h-[85vh] overflow-hidden border border-slate-100 dark:border-slate-800 mt-20 md:mt-0">
                <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-indigo-600 dark:text-indigo-400 text-[10px] font-black">{student.name as string}</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 bg-slate-200 dark:bg-slate-700 hover:bg-rose-100 hover:text-rose-600 flex items-center justify-center transition-colors"><X size={16} /></button>
                </div>
                ...
                <div className="p-4 overflow-y-auto space-y-3 bg-slate-50/50 dark:bg-slate-950">
                    {studentEvals.map((ev) => {
                        ...
                        return (
                            <div key={ev.id as string} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 shadow-sm hover:border-indigo-200 transition-all group">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {(ev.points as number) > 0 && (
                                            <span className="text-[8px] font-black text-amber-600 bg-amber-50 px-2 py-0.5 border border-amber-100">+{ev.points as number} XP</span>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[9px] font-bold text-slate-400 tabular-nums">{format(new Date(ev.created_at || ev.date), 'dd/MM/yyyy')}</span>
                                        {canDelete(ev) && (
                                ...
                                <div className="mt-2 pt-2 border-t border-slate-50 dark:border-slate-800 flex items-center gap-1.5">
                                    ...
                                    <span className="text-[8px] font-bold text-slate-400">بواسطة: {ev.teacherName as string || 'نظام آلي'}</span>
                                </div>
                            </div>
                        );
                    })}
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-center bg-white dark:bg-slate-900">
                    <button onClick={onClose} className="px-8 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-xs hover:opacity-90 transition-all">إغلاق</button>
                </div>
            </div>
        </div>
    );
