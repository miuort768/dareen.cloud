...
export const HistoryModal = ({ student, evaluations, canDelete, onDelete, onClose }: HistoryModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/50  flex items-center justify-center p-4 z-[10001] md:animate-in md:fade-in">
            <div className="bg-white dark:bg-slate-900 shadow-sm w-full max-w-xl flex flex-col max-h-[85vh] overflow-hidden border border-slate-100 dark:border-slate-800 mt-20 md:mt-0">
                <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-indigo-600 dark:text-indigo-400 text-[10px] font-medium">{student.name as string}</p>
                        </div>
                    </div>
                    ...
                    {studentEvals.map((ev) => {
                        ...
                        return (
                            <div key={ev.id as string} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 shadow-sm hover:border-indigo-200 transition-all group">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {(ev.points as number) > 0 && (
                                            <span className="text-[8px] font-medium text-amber-600 bg-amber-50 px-2 py-0.5 border border-amber-100">+{ev.points as number} XP</span>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-2">
                                ...
                                <p className="text-[10px] font-normal text-slate-600 dark:text-slate-400 italic leading-relaxed border-r-2 border-indigo-200 pr-3">
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-center bg-white dark:bg-slate-900">
                    <button onClick={onClose} className="px-8 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-medium text-xs hover:opacity-90 transition-all">إغلاق</button>
                </div>
            </div>
        </div>
    );
