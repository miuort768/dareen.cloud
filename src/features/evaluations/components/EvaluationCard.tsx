...
export const EvaluationCard = ({
  ...
  const statCells = [
    { label: 'المعدل', value: avgRating || '—', text: 'text-primary' },
    { label: 'الحضور', value: `${progress}%`, text: 'text-success' },
    { label: 'التقييمات', value: studentEvals.length, text: 'text-warning' },
  ]

  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all duration-300 hover:-translate-y-1 hover:shadow-elevation-2">
      {/* Header: avatar + name + grade + XP */}
      <div className="flex items-center gap-3 border-b border-border p-4 pb-3">
        <div
          className={cn(
            'flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br text-base font-bold shadow-md',
            gradient.g,
            gradient.on,
          )}
        >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5">
            {student.grade && (
              <span className="inline-flex items-center gap-1 rounded-lg bg-primary-soft px-2 py-0.5 text-[10px] font-bold text-primary">
      ...
      <div className="flex flex-1 flex-col gap-2.5 p-4">
        {lastEval ? (
          <>
            ...
            <div className="flex items-center justify-between gap-2 rounded-xl border border-border bg-surface px-2.5 py-2">
            ...
            <p className="line-clamp-2 rounded-xl border border-border bg-surface p-2.5 text-xs italic leading-relaxed text-muted">
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-2.5 py-5 text-center">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-dashed border-primary/30 bg-primary-soft">
            ...
            {totalEnrollments > 0 && (
              <div className="mt-1 flex flex-wrap items-center justify-center gap-1.5">
                <span className="inline-flex items-center gap-1 rounded-lg bg-primary-soft px-2 py-1 text-[10px] font-bold text-primary">
                  <BookOpen size={9} /> {totalEnrollments} مواد
                </span>
                <span className="inline-flex items-center gap-1 rounded-lg bg-info-soft px-2 py-1 text-[10px] font-bold text-info">
      ...
      <div
        className={cn(
          'grid gap-1.5 border-t border-border p-3',
      ...
      >
        {!isParent && (
          <button
            ...
            className="flex items-center justify-center gap-1 rounded-xl bg-primary py-2.5 text-[11px] font-bold text-on-primary shadow-sm transition-all hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus active:scale-95"
          >
            <Plus size={12} /> تقييم
          </button>
        )}
        ...
        >
          ...
          <span className="me-0.5 rounded-md bg-primary-soft px-1 py-0.5 text-[10px] font-bold text-primary">
            {studentEvals.length}
          </span>
        </button>
        {!isParent && (
          <button
            ...
            className="flex items-center justify-center gap-1 rounded-xl border border-border bg-surface py-2.5 text-[11px] font-bold text-main transition-all hover:border-primary/30 hover:bg-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus active:scale-95"
          >
            <User size={12} /> الملف
          </button>
        )}
