...
export const EvaluationCard = ({ student, evaluations, isParent, onAddEvaluation, onViewHistory }: EvaluationCardProps) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-sm transition-all duration-300 flex flex-col overflow-hidden group">
            ...
            <div className="p-4 flex items-center gap-3 border-b border-slate-50 dark:border-slate-800">
                ...
                <div className="shrink-0 text-center">
                    <span className="bg-amber-400/20 text-amber-700 dark:text-amber-400 text-[9px] font-medium px-2 py-0.5 border border-amber-200/50">{totalStudentXP} XP</span>
                </div>
            </div>
            ...
            {!isParent && (
                <div className="px-4 pb-4 grid grid-cols-2 gap-2">
                    <button onClick={() => onAddEvaluation(student.id as string)} className="bg-indigo-600 dark:bg-rose-500 hover:bg-indigo-700 dark:hover:bg-rose-600 text-white py-2 text-[10px] font-medium transition-all flex items-center justify-center gap-1.5 shadow-sm shadow-indigo-500/20">
                        <Plus size={12} strokeWidth={3} /> أضف تقييم
                    </button>
                    <button onClick={() => onViewHistory(student)} className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 py-2 text-[10px] font-medium transition-all flex items-center justify-center gap-1.5">
