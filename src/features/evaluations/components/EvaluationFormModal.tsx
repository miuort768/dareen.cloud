    ...
    return (
        <div className="fixed inset-0 bg-slate-900/50  flex items-center justify-center p-4 z-[10001] md:animate-in md:fade-in">
            <div className="bg-white dark:bg-slate-900 shadow-sm w-full max-w-md flex flex-col max-h-[90vh] overflow-hidden border border-slate-100/50 dark:border-slate-800/50 mt-20 md:mt-0 rounded-none">
                <div className="p-4 border-b border-slate-100/50 dark:border-slate-800/50 flex justify-between items-center bg-[#00542F] text-white">
                    ...
                    <form id="evaluation-form" onSubmit={onSubmit} className="space-y-4">
                        {!formData.studentId && (
                            <div>
                                ...
                                <select value={formData.studentId} onChange={(e) => onChange({ ...formData, studentId: e.target.value })} required
                                    className="w-full border border-slate-200 dark:border-slate-700 dark:bg-slate-800 px-3 py-2 md:px-4 md:py-3 font-normal text-xs md:text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-[#8B5CF6]/30 outline-none rounded-none">
                        ...
                        <div>
                            ...
                            <textarea value={formData.notes} onChange={(e) => onChange({ ...formData, notes: e.target.value })} rows={2}
                                className="w-full border border-slate-200 dark:border-slate-700 dark:bg-slate-800 px-3 py-2 text-sm font-normal outline-none focus:ring-2 focus:ring-[#8B5CF6]/30 resize-none placeholder:text-slate-300 transition-all rounded-none"
                ...
                <div className="p-4 border-t border-slate-100/50 dark:border-slate-800/50 flex justify-end gap-3">
                    ...
                    <button type="submit" form="evaluation-form" disabled={isSubmitting} className="px-6 py-2.5 bg-[#00542F] hover:bg-[#004028] disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-bold transition-all shadow-sm flex items-center gap-2 rounded-none">
