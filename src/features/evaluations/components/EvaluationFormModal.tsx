...
export const EvaluationFormModal = ({ isOpen, formData, students, teacherStudents, onClose, onChange, onSubmit }: EvaluationFormModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/50  flex items-center justify-center p-4 z-[10001] md:animate-in md:fade-in">
            <div className="bg-white dark:bg-slate-900 shadow-sm w-full max-w-md flex flex-col max-h-[90vh] overflow-hidden border border-slate-100 dark:border-slate-800 mt-20 md:mt-0">
                    ...
                    <form id="evaluation-form" onSubmit={onSubmit} className="space-y-4">
                        {!formData.studentId && (
                            <div>
                                ...
                                <select value={formData.studentId} onChange={(e) => onChange({ ...formData, studentId: e.target.value })} required
                                    className="w-full border border-slate-200 dark:border-slate-700 dark:bg-slate-800 px-3 py-2 md:px-4 md:py-3 font-normal text-xs md:text-sm text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500/30 outline-none">
                        ...
                        <div>
                                ...
                                <div className="flex gap-1.5">
                                    {[5, 10, 20, 50].map(p => (
                                        <button key={p} type="button" onClick={() => onChange({ ...formData, points: p })}
                                            className="bg-amber-100 text-amber-700 hover:bg-amber-200 border border-amber-200 px-2 py-0.5 font-normal text-[9px] transition-colors">+{p}</button>
                                    ))}
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <input type="number" value={formData.points || ''} onChange={(e) => onChange({ ...formData, points: Number(e.target.value) })}
                                    placeholder="0" min="0"
                                    className="flex-1 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 px-3 py-2 font-medium text-base text-amber-600 text-center outline-none focus:ring-2 focus:ring-amber-400/30" />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea value={formData.notes} onChange={(e) => onChange({ ...formData, notes: e.target.value })} rows={2}
                                className="w-full border border-slate-200 dark:border-slate-700 dark:bg-slate-800 px-3 py-2 text-sm font-normal outline-none focus:ring-2 focus:ring-indigo-500/30 resize-none placeholder:text-slate-300 transition-all"
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
                    <button type="button" onClick={onClose} className="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-white text-xs font-medium hover:bg-slate-200 transition-all">إلغاء</button>
                    <button type="submit" form="evaluation-form" className="px-6 py-2.5 bg-indigo-600 dark:bg-rose-500 hover:bg-indigo-700 dark:hover:bg-rose-600 text-white text-xs font-medium transition-all shadow-sm shadow-indigo-500/20 flex items-center gap-2">
