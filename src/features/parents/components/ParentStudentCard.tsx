        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden group hover:border-primary-500 transition-all duration-300 flex flex-col">
                ...
                <div className="relative z-10 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-primary-400 text-[9px] md:text-[10px] font-medium uppercase tracking-widest mt-0.5">{student.grade || 'غير محدد'}</p>
                        </div>
                    </div>
                    {totalPoints > 0 && (
                        <div className="flex flex-col items-center gap-0.5 bg-yellow-400 text-black px-1.5 py-1 shadow-sm transform rotate-2 shrink-0">
            ...
            <div className="grid grid-cols-2 border-b border-gray-50 dark:border-gray-800">
                ...
                <div className="p-3 md:p-4 flex flex-col items-center justify-center">
                    ...
                    <span className="text-base md:text-lg font-medium text-emerald-600">
                        {(() => {
            ...
            <div className="p-6 space-y-4 flex-1">
                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800 pb-1 flex items-center justify-between">
                        ...
                        <div key={idx} className="bg-gray-50 dark:bg-gray-800/50 p-3 relative overflow-hidden group/item">
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    ...
                                    <p className="text-[9px] text-gray-500 font-normal italic">المعلم: {en.teacher}</p>
                                </div>
                                <div className="text-left">
                                    <span className="text-[10px] font-medium text-primary-600">حضر {en.sessionsUsed} من {en.sessionsTotal}</span>
                                </div>
                            </div>
            ...
            <div className="p-4 md:p-6 pt-0 mt-auto space-y-2">
                <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => onViewDates(student)} className="py-2.5 bg-slate-900 dark:bg-slate-800 text-white text-[9px] md:text-[10px] font-medium uppercase tracking-wider rounded-none hover:bg-black transition-all flex items-center justify-center gap-1.5 md:gap-2 shadow-sm">
                        <Calendar size={13} className="md:size-[14px]" />
                        حصص الطالب
                    </button>
                    <button onClick={() => onViewAttendance(student)} className="py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white text-[9px] md:text-[10px] font-medium uppercase tracking-wider rounded-none hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex items-center justify-center gap-1.5 md:gap-2 shadow-sm">
                        <TrendingUp size={13} className="md:size-[14px]" />
                        نسبة الحضور
                    </button>
                </div>
                <button onClick={() => onViewAchievements(student)} className={cn("w-full py-2.5 text-[9px] md:text-[10px] font-medium uppercase tracking-wider rounded-none transition-all flex items-center justify-center gap-2 shadow-sm shadow-indigo-600/10", hasAchievements ? "bg-slate-100 dark:bg-slate-800 text-indigo-600 border border-indigo-200 dark:border-indigo-900" : "bg-indigo-600 text-white hover:bg-indigo-700")}>
                    <Trophy size={14} />
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800">
                        <div className="p-4 md:p-6 space-y-4">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <h4 className="text-[10px] font-medium text-slate-900 dark:text-white uppercase tracking-widest">حصاد إنجازات الطالب</h4>
                            </div>
                            <GamificationCard totalPoints={totalPoints} badges={student.badges} pointLogs={pointLogs} />
                            <button onClick={onCloseAchievements} className="w-full py-2 bg-rose-500 hover:bg-rose-600 text-white text-[8px] font-medium uppercase tracking-widest rounded-lg shadow-sm shadow-rose-500/20 transition-all mt-2 active:scale-95">إغلاق السجل</button>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
