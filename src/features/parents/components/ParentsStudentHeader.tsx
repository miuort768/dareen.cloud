    ...
    return (
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 md:px-0">
            ...
            <div className="relative group w-full md:w-72">
                <Search className="absolute end-3 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-primary transition-colors" size={16} />
                <input type="text" placeholder="بحث عن ابن..." value={searchQuery} onChange={(e) => onSearchChange(e.target.value)}
                    className="w-full pe-9 ps-4 py-2.5 bg-white/80 dark:bg-primary-active/80 backdrop-blur-xl border border-border dark:border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 font-normal transition-all text-xs shadow-sm" />
            </div>
        </div>
    );
};
