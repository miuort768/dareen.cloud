                ...
                <table className="w-full text-right border-collapse">
                    <thead>
                        <tr className="bg-gradient-to-l from-[var(--bg-primary)] to-[var(--bg-primary)]">
                            <th className="px-6 py-4 font-bold text-[9px] uppercase tracking-widest text-on-primary text-center w-16">ID</th>
                            <th className="px-6 py-4 font-bold text-[9px] uppercase tracking-widest text-on-primary">ولي الأمر</th>
                            <th className="px-6 py-4 font-bold text-[9px] uppercase tracking-widest text-on-primary">بيانات التواصل</th>
                            <th className="px-6 py-4 font-bold text-[9px] uppercase tracking-widest text-on-primary text-center">الطلاب المرتبطين</th>
                            <th className="px-6 py-4 font-bold text-[9px] uppercase tracking-widest text-on-primary text-center">الإجراءات</th>
                        </tr>
                    </thead>
                        ...
                        {parents.length > 0 ? parents.map((parent, idx) => {
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="font-medium text-[13px] text-main dark:text-on-primary uppercase tracking-tight">{parent.name}</p>
                                                <p className="text-[9px] font-normal text-muted mt-0.5 font-mono">#{parent.id.substring(0, 8)}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex flex-col gap-1.5">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-mono text-[11px] font-medium text-main dark:text-dim" dir="ltr">{parent.phone}</span>
                                            </div>
                                            {parent.email && (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] font-normal text-muted truncate max-w-[180px]">{parent.email}</span>
                                                </div>
                                            )}
                                        </div>
                                    </td>
                                        ...
                                        <div className="flex flex-col items-center gap-1.5">
                                            <div className="flex -space-x-2 space-x-reverse">
                                                {children.slice(0, 3).map((child, i) => (
                                                    <div key={i} className="w-7 h-7 bg-white dark:bg-primary-active border-2 border-border dark:border-border flex items-center justify-center text-[10px] font-medium text-primary rounded-lg">
                                                        {child.name.charAt(0)}
                                                    </div>
                                                ))}
                                                {children.length > 3 && (
                                                    <div className="w-7 h-7 bg-gradient-to-br from-[var(--bg-primary)] to-[var(--bg-primary)] border-2 border-white dark:border-border flex items-center justify-center text-[8px] font-medium text-on-primary rounded-lg">
                                                        +{children.length - 3}
                                                    </div>
                                                )}
                                            </div>
                                            <span className="text-[9px] font-medium text-muted uppercase tracking-widest">{children.length} طلاب</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                            ...
                            <tr>
                                <td colSpan={5} className="py-20 text-center">
                                    ...
                                    <p className="text-[10px] font-medium text-muted uppercase tracking-[0.2em]">لا توجد سجلات حالياً</p>
                                </td>
                            </tr>
                        )}
                    </tbody>
                    ...
                    return (
                        ...
                        >
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-1.5 mt-1">
                                            <Phone size={10} className="text-success" />
                                            <span className="text-[10px] font-medium text-muted font-mono" dir="ltr">{parent.phone}</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center justify-between mt-4 pt-4 border-t border-border dark:border-border">
                                <div className="flex items-center gap-2">
                                    <span className="text-[9px] font-bold text-muted">الأبناء</span>
                                    <div className="flex -space-x-2 space-x-reverse">
                                ...
                                <div className="flex items-center gap-1.5 px-3 py-1 shadow-sm text-on-primary text-[10px] font-bold rounded-xl bg-gradient-to-l from-[var(--bg-primary)] to-[var(--bg-primary)]">
                                    <Users size={11} />
                                    {children.length}
                                </div>
                            </div>
