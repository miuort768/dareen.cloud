...
export const SessionsModal = ({
    ...
    return (
        <div ref={containerRef} className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-12" role="dialog" aria-modal="true" aria-label={viewingSubject ? `مواعيد حصص: ${viewingSubject.subject}` : 'سجل المواعيد'} onKeyDown={handleKeyDown}>
            ...
            <div className="relative w-full max-w-lg bg-card shadow-elevation-2 rounded-2xl overflow-hidden border border-border flex flex-col max-h-[80vh] md:animate-in md:slide-in-from-bottom-8 md:duration-300">
                    ...
                    {!viewingSubject ? (
                        <div className="grid grid-cols-1 gap-3">
                            {enrollments.map((en, idx: number) => (
                                        <button key={idx} onClick={() => onSelectSubject(en)} className="p-4 bg-surface border border-border rounded-xl hover:border-primary/30 hover:bg-card transition-all text-start group flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="w-9 h-9 bg-primary-soft text-primary rounded-xl flex items-center justify-center group-hover:bg-primary group-hover:text-on-primary transition-all"><BookOpen size={16} /></div>
                                        <div>
                                    ...
                                    <ChevronLeft size={16} className="text-muted group-hover:text-primary transform group-hover:-translate-x-1 transition-all" />
                                </button>
                            ))}
                        </div>
                    ) : (
                        <div className="space-y-4">
                            <div className="flex items-center justify-between gap-2">
                                <button onClick={() => { onSelectSubject(null); onPageChange(1); }} className="inline-flex items-center gap-1.5 px-3 py-1 bg-surface border border-border rounded-xl text-micro font-medium text-primary hover:bg-primary-soft transition-all">
                            ...
                            {isSessionsLoading ? (
                                <div className="space-y-3">
                                    {[...Array(3)].map((_, i) => <div key={`parent-${i}`} className="h-16 bg-surface border border-border rounded-xl animate-pulse" />)}
                                </div>
                            ) : (
                                        ...
                                        return filtered
                                            ...
                                            .map((session, sIdx) => (
                                                <div key={sIdx} className="relative">
                                                             <div className={cn("absolute -end-[27px] top-1 w-3 h-3 rounded-full bg-card border-[3px]", session.status === 'completed' ? "border-success" : "border-error")}></div>
                                                    <div className="bg-card p-3 rounded-xl border border-border group hover:border-primary/30 transition-all">
                                                        <div className="flex items-center justify-between gap-2">
                                                            <div className="flex items-center gap-2">
