                ...
                <div className="p-4 bg-gradient-to-br from-indigo-600 to-indigo-700 text-white flex items-center justify-between shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex items-center gap-3">
                        ...
                        <div className="text-right">
                            <h2 className="text-base font-black leading-tight tracking-tight">{(viewingStudent as { name: string }).name}</h2>
                    ...
                    <div className="relative z-10 flex items-center gap-2 mr-4">
                        <div className="flex items-center gap-1 bg-white/10 px-2 py-1 rounded-lg border border-white/10">
                            <input type="date" className="bg-transparent border-none p-0 text-[10px] font-bold text-white outline-none cursor-pointer [color-scheme:dark]" value={sessionsStartDate} onChange={(e) => onStartDateChange(e.target.value)} />
                            <span className="text-[10px] text-white/60">→</span>
                            <input type="date" className="bg-transparent border-none p-0 text-[10px] font-bold text-white outline-none cursor-pointer [color-scheme:dark]" value={sessionsEndDate} onChange={(e) => onEndDateChange(e.target.value)} />
                        </div>
                    </div>
                    ...
                    {!viewingSubject ? (
                        <div className="grid grid-cols-1 gap-3">
                            {enrollments.map((en, idx: number) => (
                                <button key={idx} onClick={() => onSelectSubject(en)} className="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl hover:border-indigo-400 dark:hover:border-indigo-600 hover:shadow-md transition-all text-right group flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="w-9 h-9 bg-indigo-50 dark:bg-indigo-900/10 text-indigo-600 rounded-xl flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-all"><BookOpen size={16} /></div>
                                        <div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between gap-2">
                                <button onClick={() => { onSelectSubject(null); onPageChange(1); }} className="inline-flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-full text-[8px] font-black text-indigo-600 uppercase tracking-widest hover:bg-indigo-50 dark:hover:bg-indigo-950/20 transition-all shadow-sm">
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{sessionsPage} / {totalPages}</span>
                                            ...
                                            .map((session, sIdx) => (
                                                <div key={sIdx} className="relative">
                                                    ...
                                                    <div className="bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 group hover:border-indigo-400 dark:hover:border-indigo-600 transition-all shadow-sm">
                                                        <div className="flex items-center justify-between gap-2">
                                                            <div className="flex items-center gap-2">
                                                                <p className="text-xs font-black text-slate-900 dark:text-white">{format(new Date(session.date), 'eeee, d MMMM', { locale: ar })}</p>
                                                            </div>
                                                            <div className={cn("px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest", session.status === 'completed' ? "bg-emerald-500 text-white" : "bg-rose-500 text-white")}>
                                                        ...
                                                        {session.notes && (
                                                            <div className="mt-2 pt-2 border-t border-slate-50 dark:border-slate-800">
                                                                <div className="flex gap-1.5">
                                                                    ...
                                                                    <p className="text-[9px] text-slate-500 dark:text-slate-400 font-bold italic leading-relaxed">{session.notes}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
