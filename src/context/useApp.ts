...
const EDUCATIONAL_SUFFIX_RE = /\s*(?:لتعليم والتدريب|للتعليم والتدريب|لتعليم و التدريب)\s*$/

export const shortenAcademyName = (name: string) => name.replace(EDUCATIONAL_SUFFIX_RE, '').trim()

export const useShortAcademyName = () => shortenAcademyName(useSettingsStore((s) => s.academyName))
